...
class clsIndexBase extends clsBase
{
	function Formular()
	{
		$this->SetTitulo( "{$this->_instituicao} i-Educar - Relatório - Resumo por Mês" );
		$this->processoAp = "10010";
...
class indice extends clsCadastro
{
	...
	function renderHTML()
	{
		...
		if (is_numeric($this->ref_escola))
			$filtro .= " AND eme.ref_escola = {$this->ref_escola} ";
		...
		if ($this->total > 0) 
		{
			...
			$this->pdf = new clsPDF("Resumo por Mês - {$this->ano}", "Resumo por Mês", "A4", "", false, false);
			...
			while ( $db->ProximoRegistro() )
		 	{
			 	list($custototal,$custoporrefeicao,$caloriasdiarias,$proteinasdiarias,$custo_agrifamiliar,$mes) = $db->Tupla();
			 	$mes = $obj_envio->getMes($mes);
			 	$esquerda_aux = $esquerda = 30;
			 	$this->pdf->linha_relativa($esquerda, $altura+=18, 0, 18);
			 	$this->pdf->linha_relativa($esquerda, $altura, $direita, 0);
			 	
				$this->pdf->escreve_relativo($mes, $esquerda + 3, $altura + $altura_escrita, 200, 30, $fonte, $tam_texto, $corTexto);
	...
	function addCabecalho()
	{
		...
		$this->pdf->escreve_relativo( "Resumo por Mês - Ano: {$this->ano} / Escola: {$escola} ", 30, 78, 782, 80, $fonte, 12, $corTexto, 'center' );

		$this->pdf->escreve_relativo( "Total de Meses: {$this->total}", 30, 95, 782, 80, $fonte, 10, $corTexto, 'center' );
		...
		$this->pdf->escreve_relativo("Mês", $esquerda + 3, $altura+3, 200, 30, $fonte, $tam_texto);
		$this->pdf->linha_relativa($esquerda += 210, $altura, 0, 18);
