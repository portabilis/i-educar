...
class clsAlimentacaoProdutoUnidade{
	var $idpu;
	var $descricao;
	var $unidade;
	 ...
	 * @return object
	 */
	function clsAlimentacaoProdutoUnidade()
	{
		$db = new clsBanco();
		$this->_schema = "alimentacao.";
		$this->_tabela = "{$this->_schema}produto_unidade";
	 ...
	 * @return array
	 */
	function lista( $idpu = null)
	{
		$sql = "SELECT idpu, descricao, unidade FROM {$this->_tabela} ";

		$whereAnd = " WHERE ";

		if( is_numeric( $idpu ) )
		{
			$filtros .= "{$whereAnd} idpu = '{$idpu}'";
			$whereAnd = " AND ";
		}
