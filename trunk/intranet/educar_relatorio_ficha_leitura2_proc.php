...
/**
 * Ficha de leitura para o segundo ano.
 *
...
class indice extends clsCadastro
{
	...
	function renderHTML()
	{
		...
		$this->pdf->escreve_relativo("Leitura", 345, 142, 120, 15, null, 10);
		$this->pdf->escreve_relativo("Escrita", 405, 142, 120, 15, null, 10);
		$this->pdf->escreve_relativo("Produção de texto", 470, 142, 120, 15, null, 10);

		/***************************************/

		$imagem = girarTextoImagem("Ainda não lê", 8);
		$this->pdf->InsertJpng('png',$imagem,335,355,1);

		$imagem = girarTextoImagem("Silabando", 8);
		$this->pdf->InsertJpng('png',$imagem,350,355,1);

		$imagem = girarTextoImagem("Palavra por Palavra", 8);
		$this->pdf->InsertJpng('png',$imagem,365,355,1);

		$imagem = girarTextoImagem("Confluência", 8);
		$this->pdf->InsertJpng('png',$imagem,380,355,1);
		/*****************************************/

		/***************************************/
		$imagem = girarTextoImagem("Palavras com sílabas simples", 8);
		$this->pdf->InsertJpng('png',$imagem,395,282,1);
		$imagem = girarTextoImagem("Com erro", 8,60);
		$this->pdf->InsertJpng('png',$imagem,393,355,1);
		$imagem = girarTextoImagem("Corretas", 8,60);
		$this->pdf->InsertJpng('png',$imagem,408,355,1);

		$imagem = girarTextoImagem("Palavras com sílabas Complexas", 8);
		$this->pdf->InsertJpng('png',$imagem,425,282,1);

		$imagem = girarTextoImagem("Com Erro", 8,60);
		$this->pdf->InsertJpng('png',$imagem,425,355,1);

		$imagem = girarTextoImagem("Corretas", 8,60);
		$this->pdf->InsertJpng('png',$imagem,440,355,1);
		/*****************************************/

		/***************************************/
		$imagem = girarTextoImagem("Frases soltas dentro do tema", 8);
		$this->pdf->InsertJpng('png',$imagem,457,355,1);

		$imagem = girarTextoImagem("Texto com começo meio e fim com frases simples dentro do tema", 8,190);
		$this->pdf->InsertJpng('png',$imagem,485,355,1);

		$imagem = girarTextoImagem("Texto com frases ampliadas com desenvolvimento lógico de idéias interligadas por conectivos", 8,190);
		$this->pdf->InsertJpng('png',$imagem,520,355,1);

		$this->pdf->linha_relativa(30, 140, 540, 0);
		$this->pdf->linha_relativa(30, 140, 0, 220);
		$this->pdf->linha_relativa(570, 140, 0, 220);
		$this->pdf->linha_relativa(30, 360, 540, 0);



		$this->pdf->linha_relativa(60, 140, 0, 220);
		$this->pdf->linha_relativa(330, 140, 0, 220);

		$this->pdf->linha_relativa(330, 155, 240, 0);
		$this->pdf->linha_relativa(390, 140, 0, 220);
		$this->pdf->linha_relativa(450, 140, 0, 220);

		$this->pdf->linha_relativa(345, 155, 0, 205);
		$this->pdf->linha_relativa(360, 155, 0, 205);
		$this->pdf->linha_relativa(375, 155, 0, 205);


		$this->pdf->linha_relativa(480, 155, 0, 205);
		$this->pdf->linha_relativa(515, 155, 0, 205);


		$this->pdf->linha_relativa(420, 155, 0, 205);
		$this->pdf->linha_relativa(390, 285, 60, 0);

		$this->pdf->linha_relativa(405, 285, 0, 100);
		$this->pdf->linha_relativa(435, 285, 0, 100);

		/*****************************************/

		$this->pdf->escreve_relativo("Nº", 40, 345, 20, 20, null, 10);
		$this->pdf->escreve_relativo("Nome do aluno", 70, 345, 160, 20, null, 10);
		...
		if($lst_matricula)
		{
			foreach ($lst_matricula as $ordem => $matricula)
			{
				...
				$this->pdf->linha_relativa(345, $base+($linha*15), 0, 15);
				...
				$this->pdf->linha_relativa(515, $base+($linha*15), 0, 15);
  ...
  public function addCabecalho()
  {
    ...
    // Título
    $this->pdf->escreve_relativo("FICHA LEITURA E ESCRITA I", 30, 95, 535, 80,
      $fonte, 14, $corTexto, 'center');
    $this->pdf->escreve_relativo("Mês Referência: {$this->meses_do_ano[$this->mes]}/{$this->ano}",
