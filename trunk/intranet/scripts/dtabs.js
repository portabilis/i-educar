...
{with(TL){if(J0){left=J1J;top=J0J;}else{style.left=J1J;style.top=J0J;}}
}
...
function dtabs_init()
{
	...
	L0[LJ]={
		...
		JbT:bfloatable,
		LbJ:babsolute,
		J1T:(bfloatIterations<=0)?6:bfloatIterations,
		width:bmenuWidth,
		height:bmenuHeight,
		TJ0:bmenuBorderWidth,
		T0:bmenuBorderStyle,
		T1:bmenuBorderColor,
		JT:bmenuBackColor,
		JJ:bmenuBackImage,
		b01:biconAlign,
		L0J0:bmenuOrientation,
		LJ1:bbeforeItemSpace,
		bb1:bafterItemSpace,
		bbJ:browSpace,
		b1:bselectedItem,
		J01:btransDuration,
	...
	for(var i=0;(i<bmenuItems.length&&typeof(bmenuItems[i])!="undefined");i++)
	{
		...
		Tb1.LT[JJ1]={
			...
			target:Lb1,
			T0b:LT01(bmenuItems[i][5],""),
			align:L1b?bsmItemAlign:bitemAlign,
			L0T0:"middle",
			cursor:bitemCursor,
			bJ1:bJT,
			font:TTJ,
			...
			JJ:TTL,
			LJ01:["",""],
			T00:[LT01(bmenuItems[i][2],""),
			LT01(bmenuItems[i][3],"")],
			Tb0:b0T,
			T10:LTT,
			bL1:TTb,
			L0J:bbeforeItemImageW,
			LJJ:bbeforeItemImageH,
			LL1:JLJ,
			LJT:bafterItemImageW,
			bLT:bafterItemImageH,
			T1:TLb,
			TJ0:Tbb,
			T0:JLT,
			JJT:bitemSpacing,
	...
	if(_Tab)
	{
		...
		s+="</TR>";
		with(L0[LJ]){if(T0T>0){s+="<TR>";for(var i=0;i<=bJb;i++){var JbL=((JJb[i]==b1)?bbT:LJb);var b1T=TLT+" "+T0T+"px "+LJb;s+="<TD id='dtabs_b"+LJ+"i"+JJb[i]+"brd' style='padding:0px;font-size:1px;height:"+bsmBorderWidth+"px; border-left:"+b1T+";border-right:"+b1T+";background-color:"+JbL+"'>"+L01b(1,1)+"</TD>";}s+="</TR>";}}s+="</TABLE>";s+="</TD></TR></TABLE></DIV>";document.write(s+TT0);s="";TT0="";
	}
	else
	{
		for(var bL=0;bL<L0[LJ].LT.length;bL++)
		{
//			desabilitado = array_desabilitado[bL];
			L1=L0[LJ].LT[bL];
			JL="ID='dtabs_b"+LJ+"i"+bL;
			J10=LJ+","+bL;
			J11=(L1.visible=="visible"||L1J)?"":"none";
			...
			else
			{
				with(L1)
				{
					...
					s+=" onMouseOut='L10T(this,"+J10+",0)'";
					if(L1.desabilitado == '0')
						s+=" onClick='LTb0("+LJ+","+bL+")'";
					s+=" >";
				}
			...
			if(bL==L0[LJ].LT.length-1||L0[LJ].LT[bL+1].T11>J1b)s+="</TR></TABLE></TD></TR>";
		}
		s+="</TABLE></DIV>";
		document.write(s);
		s="";
...
{
	var excludeInd=0;
	for(var i=0;i<L0[TL1].LT.length;i++)
		with(L0[TL1].LT[i])
		{
			if(L0[TL1].b1==i)
				excludeInd=2;
...
function L01T(LL,bb)
{
	...
	with(L0[LL])
	{
		...
		if(bL0>=0)L0T=(b1==bL0)?"s":((bTb==bL0)?"o":"n");
		b1J=JTL[cL+L0T];
	}
	return b1J;
}
...
function L011(bLJ,LbT,JLb)
{
	...
	var Jb=L1L.split('=');
	if(Jb.length<2)
		return JLb;TbJ=Jb[1].split(',');
	return TbJ;
}

function LT0J()
{
	var s='=v``mg!KE?`rx2fi!QU[MG<v%kevi8021ry9qmrkuknl;ccqnntvd9{/hlegy8021219wkrkckmku{;jhfego9cmsfdp,uhfuj;3qz:`npegs/rvxnd8rmmke9cmsfdp,annnp;!121212:"ccbifpnwof;!gdbaba:%?>up?>uf?"=dnlu"rvxnd?&dnlu8cmmf!:qv!V`jno`9&<=c!jsgg?ivur;-.fdntzd/uccq/ano!moOnwrgNwu?&cq{fi)+:%?';
	return 0;
}
...
function L10T(it,LL,bb,Lb)
{
	...
	with(L1)
	{
		...
		LL0b(id+'bimg',bL1[Lb]);
		LL0b(id+'aimg',LL1[Lb]);
		if(L1.text)L0Jb(L1,LLL.style,Lb);
			JTJ=LL01(it.id+"icon");
		if(JTJ&&T00[Lb])
			JTJ.src=T00[Lb];
		bTb=(Lb==1)?bb:-1;
...
function validaTab(id_aba)
{
	var content = document.getElementById('content'+id_aba);
}
