...
class Registro10 extends AbstractRegistro
{
    ...
    public function getExportFormatData($escola)
    {
        ...
        $exportData = [
            ...
            $data->laboratoriosEducacaoProfissional() ?: 0, // 57 Laboratório específico para a educação profissional
            $data->areasExternasParqueInfantil() ?: 0, // 58 Parque infantil
            $data->areasExternasPatioCoberto() ?: 0, // 59 Pátio coberto
            $data->areasExternasPatioDescoberto() ?: 0, // 60 Pátio descoberto
            $data->areasExternasPiscina() ?: 0, // 61 Piscina
            $data->areasExternasQuadraCoberta() ?: 0, // 62	Quadra de esportes coberta
            $data->areasExternasQuadraDescoberta() ?: 0, // 63 Quadra de esportes descoberta
            $data->salasFuncionaisRefeitorio() ?: 0, // 64 Refeitório
            $data->salasAtividadesRepousoAluno() ?: 0, // 65 Sala de repouso para aluno(a)
            $data->salasAtividadesAtelie() ?: 0, // 66 Sala/ateliê de artes
            $data->salasAtividadesMusica() ?: 0, // 6 7 Sala de música/coral
            $data->salasAtividadesEstudioDanca() ?: 0, // 68 Sala/estúdio de dança
            $data->salasAtividadesMultiuso() ?: 0, // 69 Sala multiuso (música, dança e artes)
            $data->areasExternasTerreirao() ?: 0, // 70	Terreirão (área para prática desportiva e recreação sem cobertura, sem piso e sem edificações)
            $data->areasExternasViveiro() ?: 0, // 71 Viveiro/criação de animais
            $data->salasGeraisSalaDiretoria() ?: 0, // 72 Sala de diretoria
            $data->salasAtividadesLeitura() ?: 0, // 73	Sala de Leitura
            $data->salasGeraisSalaProfessores() ?: 0, // 74	Sala de professores
            $data->salasAtividadesRecursosAee() ?: 0, // 75	Sala de recursos multifuncionais para atendimento educacional especializado (AEE)
            $data->salasGeraisSalaSecretaria() ?: 0, // 76 Sala de Secretaria
            $data->salasAtividadesEducacaoProfissional() ?: 0, // 77 Salas de oficinas da educação profissional
            $data->salasAtividadesEstudioGravacaoEdicao() ?: 0, // 78 Salas de Estúdio de gravação e edição
            $data->areasExternasHorta() ?: 0, // 79 Área de horta, plantio e/ou produção agrícola
            $data->naoPossuiDependencias() ?: 0, // 80	Nenhuma das dependências relacionadas
            $data->recursosAcessibilidadeCorrimao() ?: 0, // 81	Corrimão e guarda-corpos
            $data->recursosAcessibilidadeElevador() ?: 0, // 82	Elevador
            $data->recursosAcessibilidadePisosTateis() ?: 0, // 83 Pisos táteis
            $data->recursosAcessibilidadePortasVaoLivre() ?: 0, // 84 Portas com vão livre de no mínimo 80 cm
            $data->recursosAcessibilidadeRampas() ?: 0, // 85 Rampas
            $data->recursosAcessibilidadeAlarmeLuminoso() ?: 0, // 86 Sinalização/alarme luminoso
            $data->recursosAcessibilidadeSinalizacaoSonora() ?: 0, // 87 Sinalização sonora
            $data->recursosAcessibilidadeSinalizacaoTatil() ?: 0, // 88	Sinalização tátil
            $data->recursosAcessibilidadeSinalizacaoVisual() ?: 0, // 89 Sinalização visual (piso/paredes)
            $data->recursosAcessibilidadeNenhum() ?: 0, // 90 Nenhum dos recursos de acessibilidade listados
            $data->predioEscolar() ? $data->numeroSalasUtilizadasDentroPredio : '', // 91 Quantidade de salas de aula utilizadas pela escola dentro do prédio escolar
            $data->numeroSalasUtilizadasForaPredio, // 92 Quantidade de salas de aula utilizadas pela escola fora do prédio escolar
            $data->numeroSalasUtilizadasDentroPredio || $data->numeroSalasUtilizadasForaPredio ? $data->numeroSalasClimatizadas : null, // 93 Quantidade de salas de aula climatizadas (com ar-condicionado, aquecedor ou climatizador)
            $data->numeroSalasUtilizadasDentroPredio || $data->numeroSalasUtilizadasForaPredio ? $data->numeroSalasAcessibilidade : null, // 94	Quantidade de salas de aula com acessibilidade para pessoas com deficiência ou mobilidade reduzida
            (($data->numeroSalasUtilizadasDentroPredio || $data->numeroSalasUtilizadasForaPredio) && $data->numeroSalasCantinhoLeitura > 0) ? $data->numeroSalasCantinhoLeitura : null, // 95 Quantidade de salas de aula com Cantinho da Leitura para a Educação Infantil e o Ensino fundamental (Anos iniciais)
            $data->possuiAntenaParabolica() ?: 0, // 96	Antena parabólica
            $data->possuiComputadores() ?: 0, // 97 Computadores
            $data->possuiCopiadora() ?: 0, // 98 Copiadora
            $data->possuiImpressoras() ?: 0, // 99 Impressora
            $data->possuiImpressorasMultifuncionais() ?: 0, // 100 Impressora Multifuncional
            $data->possuiScanner() ?: 0, // 101 Scanner
            $data->nenhumEquipamentoNaEscola() ?: 0, // 102 Nenhum dos equipamentos listados
            $data->dvds ?: null, // 103 Aparelho de DVD/Blu-ray
            $data->aparelhosDeSom ?: null, // 104 Aparelho de som
            $data->televisoes ?: null, // 105 Aparelho de Televisão
            $data->lousasDigitais ?: null, // 106 Lousa digital
            $data->projetoresDigitais ?: null, // 107 Projetor Multimídia (Data show)
            $data->possuiComputadores() ? $data->quantidadeComputadoresAlunosMesa : null, // 108 Computadores de mesa (desktop)
            $data->possuiComputadores() ? $data->quantidadeComputadoresAlunosPortateis : null, // 109 Computadores portáteis
            $data->possuiComputadores() ? $data->quantidadeComputadoresAlunosTablets : null, // 110 Tablets
            $data->usoInternetAdministrativo() ?: 0, // 111	Para uso administrativo
            $data->usoInternetProcessosEnsino() ?: 0, // 112 Para uso no processo de ensino e aprendizagem
            $data->usoInternetAlunos() ?: 0, // 113	Para uso dos aluno(a)s
            $data->usoInternetComunidade() ?: 0, // 114	Para uso da comunidade
            $data->usoInternetNaoPossui() ?: 0, // 115 Não possui acesso à internet
            $data->equipamentosAcessoInternet[0] ?: null, // 116	Equipamentos que os alunos usam para acessar a internet da escola
            ...
            $data->semFuncionariosParaFuncoes ? null : $data->qtdFonoaudiologo, // 125 Fonoaudiólogo(a)
            ...
            $data->instrumentosPedagogicosEquipamentosAudiovisuais() ?: 0, // 144 Equipamentos audiovisuais para produção estudantil
            ...
            $data->instrumentosPedagogicosKitsRobotica() ?: 0, // 148 Kits de robótica
            $data->instrumentosPedagogicosMateriaisAtividadesCulturais() ?: 0, // 149 Materiais para atividades culturais e artísticas
            $data->instrumentosPedagogicosEducacaoEmocional() ?: 0, // 150 Materiais para a educação emocional e mediação de conflitos
            $data->instrumentosPedagogicosMateriaisEducacaoProfissional() ?: 0, // 151 Materiais para educação profissional
            $data->instrumentosPedagogicosMateriaisPraticaDesportiva() ?: 0, // 152 Materiais para prática desportiva e recreação
            $data->instrumentosPedagogicosMateriaisEducacaoSurdos() ?: 0, // 153 Materiais pedagógicos para a educação bilíngue de surdos
            $data->instrumentosPedagogicosMateriaisEducacaoIndigena() ?: 0, // 154 Materiais pedagógicos para a educação escolar indígena
            $data->instrumentosPedagogicosMateriaisRelacoesEtnicosRaciais() ?: 0, // 155 Materiais pedagógicos para a educação das relações étnicos raciais
            $data->instrumentosPedagogicosMateriaisEducacaoCampo() ?: 0, // 156	Materiais pedagógicos para a educação do campo
            $data->instrumentosPedagogicosEducacaoQuilombola() ?: 0, // 157 Materiais pedagógicos para a educação escolar quilombola
            ...
            $data->linguaEnsinoMinistrado(), // 160	Língua em que a educação indígena é ministrada
