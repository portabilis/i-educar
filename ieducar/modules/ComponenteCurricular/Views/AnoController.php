...
class AnoController extends Core_Controller_Page_EditController
{
    ...
    public function _preConstruct()
    {
        ...
        $this->setEntries($this->getDataMapper()->findAll(
        [],
      ['componenteCurricular' => $this->getRequest()->cid]
    ));

        // Configura ação cancelar
        $this->setOptions(['url_cancelar' => [
      'path' => 'view', 'options' => [
        'query' => ['id' => $this->getRequest()->cid]
      ]
    ]]);
    }
    ...
    {
        try {
            $this->setEntity($this->getDataMapper()->createNewEntityInstance(['componenteCurricular' => $this->getRequest()->cid]));
        } catch (Exception $e) {
            $this->mensagem = $e;
