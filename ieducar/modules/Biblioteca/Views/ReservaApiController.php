...
class ReservaApiController extends ApiCoreController
{
    ...
    protected function validatesSituacaoExemplarIsIn($situacoes)
    {
        ...
        $message = sprintf(
            'Situação do exemplar deve estar em (%s)  porem atualmente é %s.',
            implode($situacoes),
            $situacaoAtual
        );
    ...
    protected function validatesNotExistsReservaEmAbertoForCliente()
    {
        if ($this->existsReservaEmAbertoForCliente()) {
            $this->messenger->append(
                'Este cliente já possui uma reserva em aberto para um exemplar desta obra.',
                'error'
            );
    ...
    */
    ...
    protected function loadExemplar($id = '', $reload = false)
    {
        if ($reload || ! isset($this->_exemplar)) {
            ...
            $this->_exemplar  = [
                'id'  => $exemplar['cod_exemplar'],
                'situacao' => $situacaoExemplar,
                'pendencias' => $this->getPendenciasForExemplar(
                    $exemplar,
                    $situacaoExemplar
                )
            ];
    ...
    protected function getPendenciasForExemplar(
        $exemplar,
        $situacaoExemplar = ''
    ) {
        ...
        if (strpos($situacaoExemplar['flag'], 'reservado') > -1) {
            $pendencias = array_merge(
                $pendencias,
                $this->loadReservasForExemplar($exemplar)
            );
        }
    ...
    protected function loadEmprestimoForExemplar($exemplar = null)
    {
        ...
        if (is_array($emprestimo) && ! empty($emprestimo)) {
            ...
            $_emprestimo['data'] = date(
                'd/m/Y',
                strtotime($emprestimo['data_retirada'])
            );

            $_emprestimo['data_prevista_disponivel'] = $this->getDataPrevistaDisponivelForExemplar(
            ...
            $_emprestimo['nome_cliente'] = $nomeCliente;
        }
    ...
    protected function loadReservasForExemplar($exemplar = null, $clienteId = null, $reload = false)
    {
        if ($reload || ! isset($this->_reservas)) {
            ...
            foreach ($reservas as $reserva) {
                ...
                // caso seja o mesmo cliente da reserva: considera a data
                // prevista disponivel gravada na reserva
                // senão considera a data prevista disponivel da reserva +
                // a quantidade de dias de emprestimo do exemplar
                if ($this->getRequest()->ref_cod_escola == $cliente['id']) {
                    $_reserva['data_prevista_disponivel'] = date(
                        'd/m/Y',
    ...
    protected function getDataPrevistaDisponivelForExemplarAfterLastPendencia(
        $exemplar = null
    ) {
        if (is_null($exemplar)) {
            $exemplar = $this->loadExemplar();
        ...
        if (!empty($exemplar['pendencias'])) {
            ...
            $dataPrevistaDisponivel = $this->getDataPrevistaDisponivelForExemplar(
                $lastPendencia['data_prevista_disponivel'],
                'Y-m-d',
                $exemplar
            );
        } else {
    ...
    protected function postReserva()
    {
        if ($this->canPostReserva()) {
            ...
            if ($reserva->cadastra()) {
                ...
                $this->messenger->append(
                    'Aparentemente a reserva não foi cadastrada, por favor, tente novamente.',
                    'error'
                );
            }
        }
        ...
        if ($this->canDeleteReserva()) {
            ...
            foreach ($reservas as $reserva) {
                ...
                if ($_reserva->excluir()) {
                    ...
                    $this->messenger->append(
                        'Aparentemente a reserva não foi cancelada, por favor, tente novamente.',
                        'error'
                    );
                }
            }
            ...
            if (empty($reservas)) {
                $this->messenger->append(
                    'Não foi encontrado uma reserva com os parâmetros recebidos.',
                    'error'
                );
            }
        }
