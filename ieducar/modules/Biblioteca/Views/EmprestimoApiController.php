...
class EmprestimoApiController extends ApiCoreController
{
    ...
    protected function validatesExistenceOfCliente()
    {
        ...
        if (!$this->loadCliente()) {
            $message = sprintf(
                "Não existe um cliente com id '%s'.",
                $this->getRequest()->cliente_id
            );
            $this->messenger->append($message);
            $valid = false;
        }
    ...
    protected function validatesClienteIsNotSuspenso()
    {
        ...
        if ($cliente['suspenso']) {
            $this->messenger->append(
                'Operação não pode ser realizada, pois o cliente esta suspenso.',
                'error'
            );
    ...
    protected function validateLimiteEmprestimoCliente()
    {
        ...
        $qtdExemplaresEmprestadosDoCliente = count(
            $exemplaresEmprestadosDoCliente
        );

        $objBiblioteca = new clsPmieducarBiblioteca(
            $this->getRequest()->biblioteca_id
        );
    ...
    protected function validatesSituacaoExemplarIsIn($expectedSituacoes)
    {
        ...
        $message = sprintf(
            "Operação não realizada, pois a situação atual do exemplar é %s quando deveria ser %s.",
            $situacao,
            implode(' ou ', $expectedSituacoes)
        );

        return $this->validator->validatesValueInSetOf(
            $situacao,
            $expectedSituacoes,
            'situação',
            false,
    ...
    protected function canPostDevolucao()
    {
        return $this->validatesPresenceOf(['exemplar_id'])
           && $this->validatesExistenceOfExemplar()
           && $this->validatesSituacaoExemplarIsIn([
               'emprestado',
    ...
    */
    ...
    protected function loadCliente($id = null)
    {
        ...
        if ($cliente) {
            $cliente = Portabilis_Array_Utils::filter(
                $cliente,
                ['cod_cliente' => 'id', 'ref_idpes' => 'pessoa_id']
            );
            ...
            $suspenso = $this->fetchPreparedQuery(
                $sql,
                $params = [$id],
                true,
                'first-field'
    ...
    protected function getDataPrevistaDisponivelForExemplar(
        ...
        $date = date(
            'Y-m-d',
            strtotime("+$qtdDiasEmprestimo days", strtotime($dataInicio))
        );
        ...
        $lst_biblioteca_dia = $obj_biblioteca_dia->lista(
            $this->getRequest()->biblioteca_id
        );
        ...
        $biblioteca_dias_folga = array_diff(
            $dias_da_semana,
            $biblioteca_dias_semana
        );
        ...
        $lst_biblioteca_feriado = $obj_biblioteca_feriado->lista(
            null,
            $this->getRequest()->biblioteca_id
        );
        ...
        if (is_array($lst_biblioteca_feriado) && count($lst_biblioteca_feriado)) {
            foreach ($lst_biblioteca_feriado as $dia_feriado) {
                ...
                $biblioteca_dias_feriado[] = dataFromPgToBr(
                    $dia_feriado['data_feriado'],
                    'D Y-m-d'
                );
            }
        }
    ...
    protected function loadReservasForExemplar(
        $exemplar,
        $clienteId = null,
        $reload = false
    ) {
        if ($reload || ! isset($this->_reservas)) {
            ...
            if ($reservas) {
                $reservas = Portabilis_Array_Utils::filterSet(
                    $reservas,
                    [
                        'cod_reserva' => 'id',
                        'data_reserva'    => 'data',
                ...
                foreach ($reservas as $index => $reserva) {
                    ...
                    $reserva['situacao'] = $this->getSituacaoForFlag(
                        'reservado'
                    );

                    // para o cliente da reserva: considera a data prevista
                    //disponivel gravada na reserva.
                    // para outros considera a data prevista disponivel da
                    // reserva + a quantidade de dias de emprestimo do exemplar
    ...
    protected function loadEmprestimoForExemplar($exemplar = null)
    {
        ...
        if ($emprestimo) {
            ...
            $emprestimo = Portabilis_Array_Utils::filter(
                $emprestimo,
                [
                    'cod_emprestimo'=> 'id',
                    'data_retirada' => 'data',
            ...
            $emprestimo['nome_cliente'] = sprintf(
                '%s - %s',
                $cliente['id'],
                $cliente['nome']
            );
            ...
            $emprestimo['data'] = date(
                'd/m/Y',
                strtotime($emprestimo['data'])
            );
    ...
    protected function existsReservaForExemplar(
        ...
        $reservas = $this->loadReservasForExemplar(
            $exemplar,
            $clienteId,
            $reload = true
        );
    ...
    protected function validateReservaOfExemplar($exemplar = null)
    {
        ...
        if ($clientePossuiReserva) {
            if ($cont==1) {
                ...
                $this->messenger->append(
                    'Outros clientes já haviam reservado o exemplar.',
                    'error'
                );
                $return = 'reservado';
            }
    ...
    protected function loadAcervo($id = '', $reload = false)
    {
        ...
        if (! isset($this->_acervos[$id])) {
            ...
            if ($acervo) {
                $acervo = Portabilis_Array_Utils::filter(
                    $acervo,
                    [
                        'cod_acervo' => 'id',
    ...
    protected function loadExemplares($reload = false, $id = null)
    {
        if ($reload || ! isset($this->_exemplares)) {
            ...
            if ($exemplares) {
                $exemplares = Portabilis_Array_Utils::filterSet(
                    $exemplares,
                    [
                        'cod_exemplar'=> 'id',
    ...
    protected function getExemplares()
    {
        $this->appendResponse(
            'exemplares',
            $this->loadExemplares($reload = true)
        );
    }

    /**
     * @param bool $permiteEmprestimo
     * @param bool $padrao
     * @param bool $emprestada
     */
    protected function loadSituacaoExemplar(
        ...
        if ($situacao) {
            $situacao = Portabilis_Array_Utils::filter(
                $situacao[0],
                [
                    'cod_situacao' => 'id',
                    'ref_cod_biblioteca' => 'biblioteca_id',
    ...
    protected function updateSituacaoExemplar($newSituacao)
    {
        if (!$newSituacao) {
            throw new CoreExt_Exception(
                '$newSituacao não pode ser falso em updateSituacaoExemplar.'
            );
        }
    ...
    protected function verificaEmprestimoEmAtraso($emprestimo)
    {
        ...
        $emprestimosEmAtraso = $emprestimo->lista(
            null,
            null,
            null,
            $this->getRequest()->cliente_id
