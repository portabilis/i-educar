...
class clsIndexBase extends clsBase
{
    ...
    {
        $this->SetTitulo(
            $this->_instituicao . ' i-Educar - Configuração movimento geral'
        );
...
class ConfiguracaoMovimentoGeralController extends clsCadastro
{
    ...
    public function Gerar()
    {
        ...
        foreach ($this->_formMap as $key => $value) {
            $this->inputsHelper()->multipleSearchSerie(
                $key,
                [
                    'label' => $value['label'],
                    'required' => false,
        ...
        foreach ($_POST as $key => $value) {
            if (strpos($key, 'multiple_search_serie_serie-') === 0) {
                ...
                foreach ($series as $serie) {
                    if (!empty($serie)) {
                        $entityInstance = $this->configDataMapper
                            ->createNewEntityInstance(
                                ['coluna' => $coluna, 'serie' => $serie]
                            );
                        $this->configDataMapper->save($entityInstance);
                    }
                }
            }
        }
