...
class VeiculoController extends Portabilis_Controller_Page_EditController
{
    ...
    {
        $this->_options = $this->mergeOptions(['edit_success' => '/intranet/transporte_veiculo_lst.php','delete_success' => '/intranet/transporte_veiculo_lst.php'], $this->_options);
    ...
    public function Gerar()
    {
        ...
        // Código do Motorista
        $options = ['label'    => $this->_getLabel('id'), 'disabled' => true,
                     'required' => false, 'size' => 25];
        $this->inputsHelper()->integer('id', $options);

        // descrição
        $options = ['label' => Portabilis_String_Utils::toLatin1($this->_getLabel('descricao')), 'required' => true, 'size' => 50, 'max_length' => 255];
        $this->inputsHelper()->text('descricao', $options);

        //placa
        $options = ['label' => Portabilis_String_Utils::toLatin1($this->_getLabel('placa')), 'required' => false, 'size' => 10, 'max_length' => 10];
        $this->inputsHelper()->text('placa', $options);

        //renavam
        $options = ['label' => Portabilis_String_Utils::toLatin1($this->_getLabel('renavam')), 'required' => false, 'size' => 15, 'max_length' => 15];
        $this->inputsHelper()->integer('renavam', $options);

        //chassi
        $options = ['label' => Portabilis_String_Utils::toLatin1($this->_getLabel('chassi')), 'required' => false, 'size' => 30, 'max_length' => 30];
        $this->inputsHelper()->text('chassi', $options);

        //marca
        $options = ['label' => Portabilis_String_Utils::toLatin1($this->_getLabel('marca')), 'required' => false, 'size' => 50, 'max_length' => 50];
        $this->inputsHelper()->text('marca', $options);

        //Ano de fabricacao
        $options = ['label' => $this->_getLabel('ano_fabricacao'), 'max_length' => 4, 'size' => 5, 'required' => false, 'placeholder' => ''];
        $this->inputsHelper()->integer('ano_fabricacao', $options);

        // Ano do modelo
        $options = ['label' => $this->_getLabel('ano_modelo'), 'max_length' => 4, 'size' => 5, 'required' => false, 'placeholder' => ''];
        $this->inputsHelper()->integer('ano_modelo', $options);

        // Passageiros
        $options = ['label' => $this->_getLabel('passageiros'), 'max_length' => 3, 'size' => 5, 'required' => true, 'placeholder' => ''];
        $this->inputsHelper()->integer('passageiros', $options);

        // Malha
        $malhas = [null           => 'Selecione uma Malha', 'A' => Portabilis_String_Utils::toLatin1('Aquaviária/Embarcação'),
      'F' => Portabilis_String_Utils::toLatin1('Ferroviária'), 'R' => Portabilis_String_Utils::toLatin1('Rodoviária')];

        $options = ['label'     => $this->_getLabel('malha'),
                     'resources' => $malhas,
                     'required'  => true];
        ...
        $options = ['label'     => $this->_getLabel('tipo'),
        ...
        // descricao_inativo
        $options = ['label' => Portabilis_String_Utils::toLatin1($this->_getLabel('descricao_inativo')), 'required' => false, 'size' => 50, 'max_length' => 155];
        ...
        // observações
        $options = ['label' => Portabilis_String_Utils::toLatin1($this->_getLabel('observacao')), 'required' => false, 'size' => 50, 'max_length' => 255];
