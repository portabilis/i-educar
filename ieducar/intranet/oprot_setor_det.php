...
class indice extends clsDetalhe
{
    ...
    public function Gerar()
    {
        ...
        if (! $detalhe) {
            $this->addDetalhe([ 'Erro', 'Setor Inexistente' ]);
        } else {
            ...
            for ($i = 0; $i < count($parentes); $i++) {
                ...
                if ($i == 0) {
                    $this->addDetalhe([ 'Setor', $strParentes ]);
                } else {
                    $this->addDetalhe([ 'Setor', "$gruda<img src=\"imagens/nvp_setal.gif\">$strParentes" ]);
                }
            }
            ...
            $this->addDetalhe([ 'Responsável pelo cadastro', $det['nome'] ]);

            $ativo = $detalhe['ativo'] == 1 ? 'Sim' : 'Não';
            $this->addDetalhe([ 'Ativo', $ativo ]);

            $no_paco = $detalhe['no_paco'] ? 'Sim' : 'Não';
            $this->addDetalhe([ 'No Paço', $no_paco ]);

            if ($detalhe['endereco']) {
                $this->addDetalhe([ 'Endereço', $detalhe['endereco'] ]);
            }

            if ($detalhe['tipo']) {
                switch ($detalhe['tipo']) {
                    case 's':
                        $this->addDetalhe([ 'Tipo', 'Secretaria' ]);
                        break;
                    case 'a':
                        $this->addDetalhe([ 'Tipo', 'Altarquia' ]);
                        break;
                    case 'f':
                        $this->addDetalhe([ 'Tipo', 'Fundação' ]);
                        break;
                }
            }
            ...
            if ($detalhe['refIdpesResp']) {
                ...
                $this->addDetalhe([ 'Secretario', $det_pessoa['nome'] ]);
            }
        }
