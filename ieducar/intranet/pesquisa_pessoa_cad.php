...
class indice extends clsCadastro
{
    ...
    public function Gerar()
    {
        ...
        if ($this->pessoa == 'FJ') {
            $this->campoRadio('pessoa', 'Tipo da pessoa:', [ 'F' => 'Física', 'J' => 'Jurídica' ], 'F');
            $this->campoOculto('pessoa_cpf', $this->pessoa_cpf);
            $this->campoOculto('vazio', 'true');
            $this->botao_enviar = false;
            $this->array_botao = [ '<< Voltar', 'Cancelar', 'Avançar >>' ];
            $this->array_botao_url_script = [ 'go( \'pesquisa_pessoa_lst.php\' );', 'window.parent.fechaExpansivel( \'div_dinamico_\' + ( parent.DOM_divs.length * 1 - 1 ) );', 'acao()' ];
            ...
            if (!$this->id_federal && !$this->cod_pessoa_fj && ($this->vazio == 'true')) {
                ...
                $selectOptions = [ null => 'Orgão emissor' ];
