...
class indice extends clsCadastro
{
    ...
    public $ref_cod_serie_disciplina;
    public $nota;
    public $faltas;
    public $disciplina_modulo;
    public $ref_cod_tipo_avaliacao;
    public $qtd_disciplinas;
    ...
    public $hora_falta;
    public $cod_disciplinas;
    public $lst_apura_falta;
    public $exame;
    public $classifica;

    public $is_nota_exame;
    public $media_especial;

    public $pula_passo;

    public $ultima_disciplina;
...
//                          $this->passo = 1;
            ...
            } elseif ($this->num_modulo > $this->max_modulos) {
...
//                                  $this->campoLista("conceito[{$matricula["cod_matricula"]}]", " &nbsp; &nbsp; &nbsp; Situação", $opcoes_conceito, "");
                                    $this->campoLista("resultado_final[{$matricula['cod_matricula']}]", '&nbsp; &nbsp; &nbsp;Resultado final', [''=>'Selecione','1'=>'Aprovado','2'=>'Reprovado'], '', '', false, '', '', false, true);
                                }
                            } else {
                                if (!is_array($lst_nota_aluno)) {
...
//                              $this->campoRotulo("espaco[{$matricula["cod_matricula"]}]", " &nbsp; &nbsp; &nbsp; Faltas","Exame n&atilde;o apura falta");
                ...
                if (is_array($lista_excessoes)) {
                    ...
                    $opcoes_excessao = [''=>'Selecione','0'=>'Reprovar','1'=>'Permitir Exame'];
                    ...
                    if (is_array($lista_excessoes)) {
                        ...
                        $opcoes_excessao = [''=>'Selecione','0'=>'Reprovar','2'=>'Aprovar'];
...
//                          else
                            $this->campoLista("resultado_final[{$matricula['cod_matricula']}]", 'Resultado final', [''=>'Selecione','1'=>'Aprovado','2'=>'Reprovado'], '', '', false, '', '', false, false);
...
//              $this->pula_passo = false;
     ...
     * @return array
     */
    public function getBoletimAluno($matricula, $ano)
    {
        $media = false;
        $reprovado = 0;
        $falta_globalizada = 0;
        $array_status = ['Aprovado','Reprovado','Reprovado por faltas'];
...
//                              echo "{$nota_modulo["ref_ref_cod_tipo_avaliacao"]},{$nota_modulo["ref_sequencial"]}\n<br>\n";
        ...
        return [ 'boletim'=>$boletim,'automatico'=>$array_status[$reprovado] ];
    }
...
//                                  echo "<pre>"; print_r($detalhe_curso); die();
                    ...
                    } elseif ($this->num_modulo - 1 == $this->max_modulos) {
            ...
            if ($this->pula_passo) {
                ...
                if (is_array($this->nota) && is_array($this->resultado_final) && is_array($this->falta)) {
                    foreach ($this->nota as $matricula => $value) {
                        $objTpAvalValores = new clsPmieducarTipoAvaliacaoValores(
                        ...
                        $objMatricula = new clsPmieducarMatricula(
                            ...
                            $this->pessoa_logada,
                                                                  null,
                            $this->resultado_final[$matricula]
                        );
                        if (!$objMatricula->edita()) {
                            die('não cadastrou (resultado final)');
                        }
                        $objFaltaAluno = new clsPmieducarFaltaAluno();
                        $existe_falta = $objFaltaAluno->lista(
                            null,
                            null,
                            null,
                            $this->ref_cod_serie_disciplina,
                        ...
                        if ($existe_falta) {
                            $objFaltaAluno = new clsPmieducarFaltaAluno(
                                $existe_falta[0]['cod_falta_aluno'],
                                $this->pessoa_logada,
                                $this->pessoa_logada,
                                $this->ref_cod_serie_disciplina,
                                                                   $this->ref_ref_cod_escola,
