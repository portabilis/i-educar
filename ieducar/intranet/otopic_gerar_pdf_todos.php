            ...
            if ($_GET['tipo'] == 'end') {
                ...
                foreach ($_SESSION['arr_pessoas'] as $indice=>$valor) {
                    ...
                    if ($valor[3] == 1) {
                        ...
                        if ($det_pessoa) {
                            $obj_relatorios->novalinha(['Nome', $det_pessoa['nome']]);
                            ...
                            if ($det_pessoa['tipo'] == 'F') {
                                ...
                                if ($detalhe) {
                                    $obj_relatorios->novalinha(['CEP', $detalhe['cep']]);
                                    ...
                                    if ($detalhe['logradouro']) {
                                        ...
                                        $obj_relatorios->novalinha(['Endereço', strtolower($detalhe['idtlog']).": {$detalhe['logradouro']} $end"]);
                                    }

                                    if ($detalhe['complemento']) {
                                        $obj_relatorios->novalinha(['Complemento', $detalhe['complemento']]);
                                    }

                                    $obj_relatorios->novalinha(['Cidade', strtolower($detalhe['cidade']).' '.strtolower($detalhe['sigla_uf'])]);

                                    //* 20-06-2006
                                    if ($detalhe['bairro']) {
                                        $obj_relatorios->novalinha(['Bairro:', $detalhe['bairro']]);
                                    }
                                    //*

                                    if ($detalhe['data_nasc']) {
                                        $obj_relatorios->novalinha(['Data Nasc', date('d/m/Y', strtotime(substr($detalhe['data_nasc'], 0, 19)))]);
                                    }
                                }
                                ...
                                $obj_relatorios->novalinha(['CEP', $cep]);

                                $obj_relatorios->novalinha(['Endereço', $endereco]);

                                if ($nm_bairro) {
                                    $this->addDetalhe(['Bairro', $nm_bairro]);
                                    $obj_relatorios->novalinha(['Bairro', $nm_bairro]);
                                }

                                $obj_relatorios->novalinha(['Cidade', $cidade]);
                            }

                            $obj_relatorios->novalinha(['', '']);
                        }
                    } else {
                        ...
                        if ($det_pessoa_auxiliar) {
                            $obj_relatorios->novalinha(['Nome:', $valor[1]]);
                            $obj_relatorios->novalinha(['CEP:', $det_pessoa_auxiliar['cep']]);
                            $obj_relatorios->novalinha(['Endereço:', "{$det_pessoa_auxiliar['logradouro']} {$det_pessoa_auxiliar['numero']}"]);

                            if ($det_pessoa_auxiliar['numero_ap']) {
                                $obj_relatorios->novalinha(['Apartamento:', $det_pessoa_auxiliar['numero_ap']]);
                            }
                            if ($det_pessoa_auxiliar['andar']) {
                                $obj_relatorios->novalinha(['Andar:', $det_pessoa_auxiliar['andar']]);
                            }
                            if ($det_pessoa_auxiliar['bloco']) {
                                $obj_relatorios->novalinha(['Bloco:', $det_pessoa_auxiliar['bloco']]);
                            }
                            if ($det_pessoa_auxiliar['letra']) {
                                $obj_relatorios->novalinha(['Letra:', $det_pessoa_auxiliar['letra']]);
                            }

                            $obj_relatorios->novalinha(['Bairro:', $det_pessoa_auxiliar['bairro']]);
                            $obj_relatorios->novalinha(['Cidade:', "{$det_pessoa_auxiliar['cidade']} {$det_pessoa_auxiliar['estado']}"]);
                            if ($det_pessoa_auxiliar['data_nasc']) {
                                $obj_relatorios->novalinha(['Data Nasc', date('d/m/Y', strtotime(substr($det_pessoa_auxiliar['data_nasc'], 0, 19)))]);
                            }
                            $obj_relatorios->novalinha(['', '']);
                        }
                    }
                ...
                foreach ($_SESSION['arr_pessoas'] as $indice=>$valor) {
                    ...
                    if ($valor[2+$i] == 1) {
                            ...
                            if ($det_pessoa['tipo'] == 'F') {
                                ...
                                if ($detalhe) {
                                    if ($detalhe['fone_1']) {
                                        ...
                                        $obj_relatorios->novalinha(['Telefone:', "{$ddd}{$detalhe['fone_1']}"]);
                                    }

                                    if ($detalhe['fone_2']) {
                                        $ddd = $detalhe['ddd_2'] ? "( {$detalhe['ddd_2']} )" : '';
                                        $obj_relatorios->novalinha(['Telefone:', "{$ddd}{$detalhe['fone_2']}"]);
                                    }

                                    if ($detalhe['fone_mov']) {
                                        $ddd = $detalhe['ddd_mov'] ? "( {$detalhe['ddd_mov']} )" : '';
                                        $obj_relatorios->novalinha(['Telefone:', "{$ddd}{$detalhe['fone_mov']}"]);
                                    }

                                    if ($detalhe['fone_fax']) {
                                        $ddd = $detalhe['ddd_fax'] ? "( {$detalhe['ddd_fax']} )" : '';
                                        $obj_relatorios->novalinha(['Telefone:', "{$ddd}{$detalhe['fone_fax']}"]);
                                    }
                                    ...
                                    if (strlen($lista[0]['obs']) < 65) {
                                        $obj_relatorios->novalinha(['Assunto:', $lista[0]['obs']]);
                                    } else {
                                        $assunto = quebra_linhas_pdf($lista[0]['obs'], 65);
                                        $obj_relatorios->novalinha(['Assunto:', $assunto], 0, 15*(count(explode("\n", $assunto))+1));
                                    }
                                }
                            } elseif ($det_pessoa['tipo'] == 'J') {
                                $objPessoaJuridica = new clsPessoaJuridica();
                                ...
                                if ($telefone_1) {
                                    ...
                                    $obj_relatorios->novalinha(['Telefone:', "{$ddd}{$telefone_1}"]);
                                }

                                if ($telefone_2) {
                                    $ddd = $ddd_telefone_2 ? "( {$ddd_telefone_2} )" : '';
                                    $obj_relatorios->novalinha(['Telefone:', "{$ddd}{$telefone_2}"]);
                                }

                                if ($telefone_mov) {
                                    $ddd = $ddd_telefone_mov ? "( {$ddd_telefone_mov} )" : '';
                                    $obj_relatorios->novalinha(['Telefone:', "{$ddd}{$telefone_mov}"]);
                                }

                                if ($telefone_fax) {
                                    $ddd = $ddd_telefone_fax ? "( {$ddd_telefone_fax} )" : '';
                                    $obj_relatorios->novalinha(['Telefone:', "{$ddd}{$telefone_fax}"]);
                                }
                        ...
                        $det_pessoa_auxiliar = $obj_pessoa_auxiliar->detalhe();
                        if ($det_pessoa_auxiliar) {
                            $obj_relatorios->novalinha(['Nome:', $valor[0+$i]]);
                            ...
                            if ($lista_tel) {
                                foreach ($lista_tel as $indice2=>$valor2) {
                                    if ($valor2['fone']) {
                                        ...
                                        $obj_relatorios->novalinha(['Telefone:', "{$d}{$valor2['fone']}"]);
                                    }
                                }
                            }
