			...
			if(!$curso_conceitual)
			{
				...
				foreach ($lista_matricula as $matricula)
			    {
				    	...
				    	if(!$this->em_branco)
				    	{
							if($det_curso['falta_ch_globalizada'])
							{
								...
								if(is_array($det_falta))
								{
									...
									echo "<pre>"; print_r($det_falta); die();
						...
						if($det_nota){
							...
							foreach ($det_nota as $key => $nota) {
								...
								else
								{
									...
									{
										$nota = sprintf("%01.1f",$det_nota[$key]['nota']);
					...
					if($completo){
						...
						$sit = ($det_matricula['aprovado'] == 3)? "EM ANDAMENTO" : $sit;
						$sit = ($det_matricula['aprovado'] == 7)? "EM EXAME" : $sit;
