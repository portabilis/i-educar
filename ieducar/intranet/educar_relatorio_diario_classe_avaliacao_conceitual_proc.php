...
class indice extends clsCadastro
{
  ...
  function renderHTML()
  {
    ...
    $this->ref_cod_modulo = $this->ref_cod_modulo[1];
    ...
    $prox_mes = $this->mes + 1;
    $this->pdf = new clsPDF(sprintf('Diário de Classe - %s', $this->ano),
      sprintf('Diário de Classe - %s e %s de %s', $this->meses_do_ano[$this->mes], $this->meses_do_ano[$prox_mes], $this->ano),
      'A4', '', FALSE, FALSE);
    ...
    if ($lista_matricula || $this->em_branco) {
      ...
      foreach ($lista_matricula as $matricula) {
        ...
        $this->pdf->escreve_relativo(sprintf('%02d', $num_aluno), 38, $this->page_y + 4,
          30, 15, $fonte, 7, $corTexto, 'left');

        $this->pdf->escreve_relativo($matricula['nome_aluno'], 55, $this->page_y + 4,
          160, 15, $fonte, 7, $corTexto, 'left');
  ...
  function desenhaLinhasVertical()
  {
    ...
    for ($linha = 0; $linha < $this->total - 1; $linha++) {
      ...
      $this->pdf->escreve_relativo($linha + 1, $reta_ano_x, 145, $incremental,
        80, $fonte, 7, $corTexto, 'center');
...
ND = Não Desenvolvida
CNA = Competência Não Avaliada", 36,750, 200, 50, $fonte, 7, $corTexto, 'left');

    $this->pdf->escreve_relativo('Data: ____/____/_______', 200, 810, 100, 50,
      $fonte, 7, $corTexto, 'left');

    $this->pdf->escreve_relativo('Assinatura do Professor(a)', 450, 780, 100, 50,
      $fonte, 7, $corTexto, 'left');

    $this->pdf->linha_relativa(430, 775, 130, 0);
  }
