    ...
    public function Gerar()
    {
        ...
        if ($dbms->ProximoRegistro()) {
            ...
            if ($data_inicial_a < 2005) {
                ...
                $data_final = '';
                $this->addDetalhe(['Extra', $data_inicial]);
            } else {
                $this->addDetalhe(['Inicio período da folha', $data_inicial]);
                $this->addDetalhe(['Fim período da folha', $data_final]);
                $data_final_compara_2 = date('Y-m-d');
                if ($data_final_compara>$data_final_compara_2) {
                    $this->addDetalhe(['<span style=\'color: red;\'>Aviso</span>', '<h3 style=\'color: red;\'>Os valores poderão ser alterados até o último dia do período da folha.</h3>']);
                }
            }
        }
        $dbms->Libera();
        ...
        while ($dbms->ProximoRegistro()) {
            ...
            if ($tipo_evento > 3) {
                //
            } elseif ($tipo_evento == 1 || $tipo_evento == 2) {
        ...
        $this->addDetalhe(['Descrição', $descricao_holerite]);
