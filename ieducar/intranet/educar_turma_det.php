    ...
    public function Gerar()
    {
      ...
      null,
        ...
        if ($registro['ref_cod_regente']) {
            ...
            $this->addDetalhe(['Professor/Regente', $det['nome']]);
        }

        if ($registro['ref_cod_infra_predio_comodo']) {
            $this->addDetalhe(['Sala', $registro['ref_cod_infra_predio_comodo']]);
        }

        if ($registro['ref_cod_turma_tipo']) {
            $this->addDetalhe(['Tipo de Turma', $registro['ref_cod_turma_tipo']]);
        }
        ...
        if ($registro['sgl_turma']) {
            $this->addDetalhe([_cl('turma.detalhe.sigla'), $registro['sgl_turma']]);
        }
        ...
        $this->addDetalhe(['Situação', dbBool($registro['visivel']) ? 'Ativo' : 'Desativo']);
        ...
        if ($padrao_ano_escolar == 1) {
            ...
            if (is_string($registro['dias_semana']) && !empty($registro['dias_semana'])) {
                ...
                $this->addDetalhe(['Dia da Semana', $diasSemana]);
            }
      ...
      <table>
        ...
        $tabela3 .= '</table>';
        $this->addDetalhe(['Componentes curriculares', $tabela3]);
