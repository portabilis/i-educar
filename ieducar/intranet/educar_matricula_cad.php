              ...
              if ($editou2) {
                ...
                if ($det_matricula['aprovado'] == 3) {
                  ...
                  if ($editou_mat) {
                    ...
                    $lst_esd = $obj_esd->lista(
                      $this->ref_ref_cod_serie, $this->ref_cod_escola, NULL, 1
                    );
                    ...
                    // Lista as notas que o aluno já tinha
                    $obj_nota_aluno = new clsPmieducarNotaAluno();
                    $lst_nota_aluno = $obj_nota_aluno->lista(
                      NULL, NULL, NULL, $this->ref_ref_cod_serie, NULL, NULL,
                      $matricula_saida, NULL, NULL, NULL, NULL, NULL, NULL, 1
                    );

                    if (is_array($lst_nota_aluno)) {
                      foreach ($lst_nota_aluno as $campo) {
                        if ($disciplinas[$campo['ref_cod_disciplina']]) {
                          // Cadastra as notas existentes na nova matrícula
                          $obj_nota_aluno = new clsPmieducarNotaAluno(
                            NULL, $campo['ref_sequencial'],
                            $campo['ref_ref_cod_tipo_avaliacao'], $this->ref_ref_cod_serie,
                            $this->ref_cod_escola, $campo['ref_cod_disciplina'],
                            $cadastrou, NULL, $this->pessoa_logada, NULL, NULL,
                            1, $campo['modulo']
                          );
                    ...
                    // Lista as faltas que o aluno já tinha
                    $obj_falta_aluno = new clsPmieducarFaltaAluno();
                    $lst_falta_aluno = $obj_falta_aluno->lista(
                      NULL, NULL, NULL, $this->ref_ref_cod_serie, NULL, NULL,
                      $matricula_saida, NULL, NULL, NULL, NULL, NULL, 1
                    );

                    if (is_array($lst_falta_aluno)) {
                      foreach ($lst_falta_aluno as $campo) {
                        if ($disciplinas[$campo['ref_cod_disciplina']]) {
                          // Cadastra as faltas existentes na nova matrícula
                          $obj_falta_aluno = new clsPmieducarFaltaAluno(
                            NULL, NULL, $this->pessoa_logada,
                            $this->ref_ref_cod_serie, $this->ref_cod_escola,
                            $campo['ref_cod_disciplina'], $cadastrou,
                            $campo['faltas'], NULL, NULL, 1, $campo['modulo']
                          );
                    ...
                    // Lista as notas que o aluno já tinha (globalizada)
                    ...
                    if (is_array($lst_faltas)) {
                      foreach ($lst_faltas as $campo) {
                        // Cadastra as faltas existentes na nova matrícula
                        $obj_faltas = new clsPmieducarFaltas(
                          $cadastrou, $campo['sequencial'], $this->pessoa_logada,
                          $campo['falta']
                        );
