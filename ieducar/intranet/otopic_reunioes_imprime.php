...
class indice extends clsListagem
{
    public function Gerar()
    {
        ...
        $objRelatorio->novalinha(['Informações Gerais:'], 0, 16, true, 'arial', false, '#FFFFFF', false, '#000000');
        $objRelatorioCor->novalinha(['Informações Gerais:'], 0, 16, true);
        ...
        //pdf
        $objRelatorio->novalinha(['Descrição:', quebra_linhas_pdf($detalhe['descricao'], 70) ], 0, 13*(count(explode("\n", quebra_linhas_pdf($detalhe['descricao'], 70)))), false, false, 107, false, '#FFFFFF');
        $objRelatorioCor->novalinha(['Descrição:', quebra_linhas_pdf($detalhe['descricao'], 70) ], 0, 13*(count(explode("\n", quebra_linhas_pdf($detalhe['descricao'], 70)))), false, false, 107);
        ...
        if ((substr($detalhe['data_inicio_marcado'], 0, 10) == substr($detalhe['data_fim_marcado'], 0, 10))) {
            $objRelatorio->novalinha(['Data Marcada:', date('d/m/Y H:i:s', strtotime(substr($detalhe['data_inicio_marcado'], 0))).' as '.date('H:i:s', strtotime(substr($detalhe['data_fim_marcado'], 10))) ], 0, 13, false, false, 108, false, '#FFFFFF');
            $objRelatorioCor->novalinha(['Data Marcada:', date('d/m/Y H:i:s', strtotime(substr($detalhe['data_inicio_marcado'], 0))).' as '.date('H:i:s', strtotime(substr($detalhe['data_fim_marcado'], 10, 19))) ], 0, 13, false, false, 108);
        } else {
            $objRelatorio->novalinha(['Data Marcada:', date('d/m/Y H:i:s', strtotime(substr($detalhe['data_inicio_marcado'], 0))).' - '.date('d/m/Y H:i:s', strtotime(substr($detalhe['data_fim_marcado'], 0))) ], 0, 13, false, false, 109, false, '#FFFFFF');
            $objRelatorioCor->novalinha(['Data Marcada:', date('d/m/Y H:i:s', strtotime(substr($detalhe['data_inicio_marcado'], 0))).' - '.date('d/m/Y H:i:s', strtotime(substr($detalhe['data_fim_marcado'], 0))) ], 0, 13, false, false, 109);
        }
        ...
        if (($detalhe['data_inicio_marcado'] != $detalhe['data_inicio_real']) || ($detalhe['data_fim_marcado'] != $detalhe['data_fim_real'])) {
            if ((substr($detalhe['data_inicio_real'], 0, 10) == substr($detalhe['data_fim_real'], 0, 10))) {
                if ($detalhe['data_fim_real']) {
                    $objRelatorio->novalinha(['Data da execução:', date('d/m/Y H:i:s', strtotime(substr($detalhe['data_inicio_real'], 0))).' as '.date('H:i:s', strtotime(substr($detalhe['data_fim_real'], 10))) ], 0, 13, false, false, 109, false, '#FFFFFF');
                    $objRelatorioCor->novalinha(['Data da execução:', date('d/m/Y H:i:s', strtotime(substr($detalhe['data_inicio_real'], 0))).' as '.date('H:i:s', strtotime(substr($detalhe['data_fim_real'], 10))) ], 0, 13, false, false, 109);
                }
            } else {
                if ($detalhe['data_fim_real']) {
                    $objRelatorio->novalinha(['Data da execução:', date('d/m/Y H:i:s', strtotime(substr($detalhe['data_inicio_real'], 0))).' - '.date('d/m/Y H:i', strtotime(substr($detalhe['data_fim_real'], 0))) ], 0, 13, false, false, 109, false, '#FFFFFF');
                    $objRelatorioCor->novalinha(['Data da execução:', date('d/m/Y H:i:s', strtotime(substr($detalhe['data_inicio_real'], 0))).' - '.date('d/m/Y H:i', strtotime(substr($detalhe['data_fim_real'], 0))) ], 0, 13, false, false, 109);
                }
            }
        }
        ...
        if (!$detalhe['data_fim_real']) {
            ...
            $objRelatorio->novalinha(['Notificado por e-mail:', $notificacaoPorEmail], 0, 13, false, false, 110, false, '#FFFFFF');
            $objRelatorioCor->novalinha(['Notificado por e-mail:', $notificacaoPorEmail], 0, 13, false, false, 110, false, '#FFFFFF');
        } else {
        ...
        if ($detalhe['data_inicio_real'] && !$detalhe['data_fim_real']) {
            ...
            $notificacaoPorEmail = ($detalhe['email_enviado']) ? 'Sim' : 'Não';
            $objRelatorio->novalinha(['Notificado por e-mail:', $notificacaoPorEmail], 0, 13, false, false, 110, false, '#FFFFFF');
            $objRelatorio->novalinha(['Tempo de Duração:', $dif.'   '.'Notificado por e-mail: '.$notificacaoPorEmail ], 0, 13, false, false, 109, false, '#FFFFFF');
            $objRelatorioCor->novalinha(['Tempo de Duração:', $dif.'   '.'Notificado por e-mail: '.$notificacaoPorEmail ], 0, 13, false, false, 109);
        ...
        if ($detalhe['data_fim_real']) {
            ...
            //pdf
            $notificacaoPorEmail = ($detalhe['email_enviado']) ? 'Sim' : 'Não';
            $objRelatorio->novalinha(['Tempo de Duração:', difTempo($data_inicial, $data_final).'   '.'Notificado por e-mail: '.$notificacaoPorEmail ], 0, 13, false, false, 109, false, '#FFFFFF');
            $objRelatorioCor->novalinha(['Tempo de Duração:', difTempo($data_inicial, $data_final).'   '.'Notificado por e-mail: '.$notificacaoPorEmail ], 0, 13, false, false, 109);
        }
        ...
        //pdf
        $objRelatorio->novalinha(['Tópicos Relacionados:'], 0, 13, true, 'arial', false, '#FFFFFF', false, '#000000');
        $objRelatorioCor->novalinha(['Tópicos Relacionados:'], 0, 13, true);
        ...
        foreach ($lista as $topicos) {
            ...
            $objRelatorio->novalinha(['Assunto:', quebra_linhas_pdf($auxAssunto, 70) ], 0, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 108, false, '#FFFFFF');
            $objRelatorioCor->novalinha(['Assunto:', quebra_linhas_pdf($auxAssunto, 70) ], 0, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 108);
            ...
            if ($auxAssunto) {
                //pdf
                $objRelatorio->novalinha(['Parecer Atual:', quebra_linhas_pdf($auxAssunto, 60) ], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93, false, '#FFFFFF');
                $objRelatorioCor->novalinha(['Parecer Atual:', quebra_linhas_pdf($auxAssunto, 60) ], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93);
            }
            ...
            if (count($lista_topico_reuniao)> 1 && is_array($lista_topico_reuniao)) {
                foreach ($lista_topico_reuniao as $parecer) {
                    if ($parecer['parecer'] && $parecer['ref_cod_reuniao'] != $cod_reuniao) {
                        ...
                        $objRelatorio->novalinha(['Outros Pareceres:', quebra_linhas_pdf($auxAssunto, 60) ], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93, false, '#FFFFFF');
                        $objRelatorioCor->novalinha(['Outros Pareceres:', quebra_linhas_pdf($auxAssunto, 60) ], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93);
                    }
                }
            }
            if (!$detalhe['data_fim_real']) {
                $objRelatorio->novalinha([''], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93, false, '#FFFFFF');
                $objRelatorio->novalinha([''], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93, false, '#FFFFFF');
                $objRelatorio->novalinha([''], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93, false, '#FFFFFF');
                $objRelatorioCor->novalinha([''], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93);
                $objRelatorioCor->novalinha([''], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93);
                $objRelatorioCor->novalinha([''], 15, 13*(count(explode("\n", quebra_linhas_pdf($auxAssunto, 70)))), false, false, 93);
            }
        ...
        */
        if ($detalhe['data_fim_real']) {
            ...
            if ($listaParticipantes) {
                //pdf
                $objRelatorio->novalinha([], 0, 5, true, 'arial', false, '#FFFFFF', false, '#000000');
                $objRelatorio->novalinha(['Participantes:'], 0, 16, true, 'arial', false, '#FFFFFF', false, '#000000');
                $objRelatorioCor->novalinha(['Participantes:'], 0, 16, true, 'arial');
                foreach ($listaParticipantes as $participante) {
                    ...
                    if (substr($participante['data_chegada'], 0, 10) == substr($participante['data_saida'], 0, 10)) {
                        $objRelatorio->novalinha([$detPessoaFisica['nome'], date('d/m/Y H:i:s', strtotime(substr($participante['data_chegada'], 0, 18))).' as '.date('H:i:s', strtotime(substr($participante['data_saida'], 10))) ], 15, 13, false, false, 205, false, '#FFFFFF');
                        $objRelatorioCor->novalinha([$detPessoaFisica['nome'], date('d/m/Y H:i:s', strtotime(substr($participante['data_chegada'], 0, 18))).' as '.date('H:i:s', strtotime(substr($participante['data_saida'], 10))) ], 15, 13, false, false, 205);
                    } else {
                        $objRelatorio->novalinha([$detPessoaFisica['nome'], date('d/m/Y H:i:s', strtotime(substr($participante['data_chegada'], 0, 18))).' - '.date('d/m/Y H:i:s', strtotime(substr($participante['data_saida'], 0, 18))) ], 15, 13, false, false, 205, false, '#FFFFFF');
                        $objRelatorioCor->novalinha([$detPessoaFisica['nome'], date('d/m/Y H:i:s', strtotime(substr($participante['data_chegada'], 0, 18))).' - '.date('d/m/Y H:i:s', strtotime(substr($participante['data_saida'], 0, 18))) ], 15, 13, false, false, 205);
                    }
                }
            }
        }
        ...
        if ($_GET['imprimir'] == 'jato') {
            ...
            $this->addLinhas(["<a href=$linkCor>Clique aqui para abrir o arquivo</a>"]);
        }
        $this->array_botao = ['Cancelar'];
        $this->array_botao_url = ["otopic_reunioes_det.php?cod_reuniao=$cod_reuniao&cod_grupo=$cod_grupo"];
