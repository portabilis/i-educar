...
class indice extends clsDetalhe
{
    ...
    public function Gerar()
    {
        ...
        if ($registro['ref_cod_material_tipo']) {
            $this->addDetalhe([ 'Tipo de Material', "{$registro['ref_cod_material_tipo']}"]);
        }
        if ($registro['nm_material']) {
            $this->addDetalhe([ 'Material', "{$registro['nm_material']}"]);
        }
        if ($registro['desc_material']) {
            $this->addDetalhe([ 'Descri&ccedil;&atilde;o', "{$registro['desc_material']}"]);
        }
        if ($registro['custo_unitario']) {
            $valor = number_format($registro['custo_unitario'], 2, ',', '.');
            $this->addDetalhe([ 'Custo Unit&aacute;rio', "{$valor}"]);
        }
