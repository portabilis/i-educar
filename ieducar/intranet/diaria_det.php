...
class indice extends clsDetalhe
{
    public function Gerar()
    {
        ...
        if ($db->ProximoRegistro()) {
            ...
            $this->addDetalhe([ 'N&deg;. do roteiro', "<span style=\"font-size: 14px;font-weight:bold;\">{$cod_diaria}</span>" ]);
            ...
            $detalhePessoa = $objPessoa->detalhe();
            $this->addDetalhe([ 'Funcion&aacute;rio', $detalhePessoa['nome'] ]);
            $this->addDetalhe([ 'CPF', int2CPF($detalhePessoa['cpf']) ]);

            $objFuncionario = new clsFuncionario($ref_funcionario);
            $detalheFuncionario = $objFuncionario->detalhe();
            $this->addDetalhe([ 'Matricula', $detalheFuncionario['matricula'] ]);
            ...
            if ($ref_cod_setor) {
                ...
                $this->addDetalhe([ 'Secretaria', $nm_secretaria ]);
            }

            $this->addDetalhe([ 'Conta Corrente', $conta_corrente ]);
            if (strlen($agencia) < 5) {
                $agencia = str_repeat('0', 5 - strlen($agencia)) . $agencia;
            }
            if ($agencia) {
                $this->addDetalhe([ 'Ag&ecirc;ncia', $agencia ]);
            }
            ...
            if ($banco) {
                $this->addDetalhe([ 'Banco', $banco ]);
            }
            if ($dotacao_orcamentaria) {
                $this->addDetalhe([ 'Dota&ccedil;&atilde;o or&ccedil;ament&aacute;ria', $dotacao_orcamentaria ]);
            }
            if ($objetivo) {
                $this->addDetalhe([ 'Objetivo', $objetivo ]);
            }

            if ($data_pedido) {
                $this->addDetalhe([ 'Data Pedido', date('d/m/Y', strtotime(substr($data_pedido, 0, 16))) ]);
            }

            if ($data_partida) {
                $this->addDetalhe([ 'Data Partida', date('d/m/Y H:i', strtotime(substr($data_partida, 0, 16))) ]);
            }

            if ($data_chegada) {
                $this->addDetalhe([ 'Data Chegada', date('d/m/Y H:i', strtotime(substr($data_chegada, 0, 16))) ]);
            }
            ...
            if ($destino) {
                $this->addDetalhe([ 'Destino', $destino ]);
            }

            $this->addDetalhe([ '100%', number_format($vl100, 2, ',', '.') ]);
            $this->addDetalhe([ '75%', number_format($vl75, 2, ',', '.') ]);
            $this->addDetalhe([ '50%', number_format($vl50, 2, ',', '.') ]);
            $this->addDetalhe([ '25%', number_format($vl25, 2, ',', '.') ]);
            $this->addDetalhe([ 'Total', '<span style="font-size: 12px;font-weight:bold;border-width:1px;border-color:#000000;border-style:solid;padding:2px;">' . number_format($vl25 + $vl50 + $vl75 + $vl100, 2, ',', '.') . '</span>' ]);

            if ($num_diaria) {
                $num_diaria = sprintf('%06d', $num_diaria);
                $this->addDetalhe([ 'Nº Di&aacute;ria', "{$num_diaria}/{$data['2']}" ]);
            }
