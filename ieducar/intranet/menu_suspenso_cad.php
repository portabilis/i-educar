	...
	function Inicializar()
	{
		 ...
		 if($this->cod_menu && !$_POST)
		 {
			...
			if ($lista)
			{
		 			...
		 			{
		 						$_SESSION['menu_suspenso'][]  = array("ref_cod_menu_pai"=>$menu['ref_cod_menu_pai'],
		 ...
		 if($_FILES['ico_menu']['name'])
		 {
			...
			$extensao = substr($_FILES['ico_menu']['name'], -3);
	 	    if(move_uploaded_file($_FILES['ico_menu']['tmp_name'], $caminho.$nome_do_arquivo))
			{
					$objImagem = new clsPortalImagem(false, 1, $nome_do_arquivo, false,$extensao,  false, false, false, $this->id_pessoa);
					if($cod_imagem = $objImagem->cadastra())
					{
						$this->ico_menu = $cod_imagem;
			   			echo "<script>alert(\"Icone inserido!\")</script>";
					}
			}
		 }
		 ...
		 elseif (!$_POST['lista'] && $_POST)
		 {
		 	...
		 	if($_POST['editando']==1)
		 	{
				...
				//***
			 		...
			 		$_SESSION['menu_suspenso'][$_POST['editar']]  = array("ref_cod_menu_pai"=>$_POST['ref_cod_menu_pai'],
			 	...
			 	{
			 		echo "<script>alert ('Os campos \"Ordem\" e \"Tï¿½tulo\" sï¿½o obrigatï¿½rios!');</script>";
			 	}
		 	...
		 	else
		 	{
				...
				//***
			 		...
			 		$icone = ($_POST['img_banco']) ? @$_POST['img_banco'] : $cod_imagem;
			 		$_SESSION['menu_suspenso'][]  = array("ref_cod_menu_pai"=>$_POST['ref_cod_menu_pai'],
			 										"cod_menu"=> $this->cod_menu,
													"ref_cod_menu_submenu"=>$_POST['ref_cod_menu_submenu'],
		...
		if($_GET &&  $_SESSION['menu_suspenso'])
		{
			foreach ($_SESSION['menu_suspenso'] as $id => $value)
			{
				...
				if ($menu /*&& !$menu_pai*/)
				{
					...
					$db->Consulta(" SELECT ref_cod_menu_menu
									       ,nm_menu
									  FROM menu_submenu
									       ,menu_menu
									 WHERE cod_menu_menu    = ref_cod_menu_menu
									   AND cod_menu_submenu = {$menu}");
/*
									union
									SELECT cod_menu_menu
									       ,nm_menu
									  FROM menu_menu
									 WHERE ref_cod_menu_pai = (SELECT ref_cod_menu_menu
															     FROM menu_submenu
																      ,menu_menu
															    WHERE cod_menu_menu    = ref_cod_menu_menu
															      AND cod_menu_submenu = 294)
*/
					if($db->ProximoRegistro())
					{//print_r($db->Tupla());die;
			...
			/*		$num_rows = $db->Consulta("SELECT cod_menu_menu
									       ,nm_menu
									  FROM menu_menu
									 WHERE ref_cod_menu_pai = '{$cod_sub}'") ;
					...
					$union = " union
								SELECT cod_menu_submenu, nm_submenu FROM menu_submenu WHERE cod_sistema = 2 $where_filho $menu_suspenso ORDER BY nm_submenu ";
					}else {

						$union = " ORDER BY nm_submenu ";
	...
	if($cod_menu){
		...
		$num_rows = $db->Consulta("SELECT cod_menu_menu
						       ,nm_menu
						  FROM menu_menu
						 WHERE ref_cod_menu_pai = '{$cod_menu}'") ;
		...
		if(pg_num_rows($num_rows) ){
			...
			if($_SESSION['menu_suspenso'])
			{
				foreach ($_SESSION['menu_suspenso'] as $campo)
				{
					...
					{//continua amanha $db->CampoUnico("Select ref_cod_menu_pai from menu_menu,menu_submenu where")
						$AND = "AND";
			...
			///////
		$union = " union
					SELECT cod_menu_submenu, nm_submenu,1 FROM menu_submenu WHERE cod_sistema = 2 $where_filho $menu_suspenso_filho $menu_suspenso";
		}else {
	...
	//**************
		...
		else
		{
			...
			$db1->Consulta("SELECT cod_menu_submenu, nm_submenu,0 FROM menu_submenu WHERE cod_sistema = 2 $where $menu_suspenso
							$union");
		/*	echo "<pre>SELECT cod_menu_submenu, nm_submenu,0 FROM menu_submenu WHERE cod_sistema = 2 $where $menu_suspenso
						$union";die;*/
			while ($db1->ProximoRegistro())
			{
		...
		$lista_sim_nao = array("1"=>"Sim", "0"=>"N&atilde;o");
	...
	function Editar()
	{
		...
		$filhos = array();
		//echo '<pre>';print_r($_SESSION['menu_suspenso']);die;
