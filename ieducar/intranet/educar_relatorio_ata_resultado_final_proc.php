...
class indice extends clsCadastro
{
  ...
  function renderHTML()
  {
      ...
      is_numeric($this->ref_cod_turma) && is_numeric($this->ano))
    {
      ...
      if ($db->numLinhas()) {
        ...
        $lst_escola_serie_disciplina = $obj_escola_serie_disciplina->lista(
        ...
        echo "
          <script>
            window.onload=function() {
              ...
              window.location='download.php?filename=".$this->get_link."'
            }
          </script>";
        ...
        echo "
          <html>
            <center>
              Se o download não iniciar automaticamente <br /><a target='_blank' href='" . $this->get_link  . "' style='font-size: 16px; color: #000000; text-decoration: underline;'>clique aqui!</a><br><br>
  ...
  function montaAlunoNaoGlobalizados($alunos_matriculados, $det_curso, $curso_conceitual)
  {
    ...
    foreach ($alunos_matriculados as $matricula => $aluno) {
      ...
      $this->pdf->escreve_relativo($aluno['aprovado'] == 1 ? 'Aprovado' : 'Reprovado',
      ...
      foreach ($this->disciplinas as $cod_disciplina => $disciplina) {
        $obj_dispensa_disciplina = new clsPmieducarDispensaDisciplina($matricula,
          $aluno['ref_cod_serie'], $this->ref_cod_escola, $cod_disciplina);
        ...
        if ($obj_dispensa_disciplina->detalhe()) {
          ...
          if ($curso_conceitual) {
            ...
            if (dbBool($det_serie['ultima_nota_define'])) {
              ...
              $media = $obj_nota_aluno->getUltimaNotaModulo($matricula, $cod_disciplina,
                $aluno['ref_cod_serie'], count($this->array_modulos));
            } else {
              $obj_nota_aluno = new clsPmieducarNotaAluno();
              $obj_nota_aluno->setOrderby('modulo ASC');
              ...
              $lst_nota_aluno = $obj_nota_aluno->lista(NULL, NULL, NULL,
                $aluno['ref_cod_serie'], $this->ref_cod_escola, $cod_disciplina,
                $matricula, NULL, NULL, NULL, NULL, NULL, NULL, 1);
              ...
              $media = $nota_exame ? $notas / (count($lst_nota_aluno) + 1) :
                $notas / count($lst_nota_aluno);
        ...
        $this->pdf->escreve_relativo($media, $esquerda, $this->page_y + 4,
          $espaco_disciplinas / 2, 100, $fonte, $tam_texto + 1, $corTexto, 'center');

        $this->pdf->linha_relativa($esquerda + $espaco_disciplinas / 2,
          $this->page_y, 0, $altura);

        $this->pdf->escreve_relativo($faltas, $esquerda + $espaco_disciplinas / 2,
          $this->page_y + 4, $espaco_disciplinas / 2, 100, $fonte, $tam_texto + 1,
          $corTexto, 'center');
      ...
      foreach ($this->disciplinas as $cod_disciplina => $disciplina) {
        ...
        if ($obj_dispensa_disciplina->detalhe()) {
          ...
          if ($curso_conceitual) {
            $obj_nota_aluno = new clsPmieducarNotaAluno();
            $obj_nota_aluno->setOrderby('modulo ASC');
            $lst_nota_aluno = $obj_nota_aluno->lista(NULL, NULL,
              $det_curso['ref_cod_tipo_avaliacao'], $aluno['ref_cod_serie'],
              $this->ref_cod_escola, $cod_disciplina, $matricula, NULL, NULL,
              NULL, NULL, NULL, NULL, 1);
            ...
            if (dbBool($det_serie['ultima_nota_define'])) {
              ...
              $media = $obj_nota_aluno->getUltimaNotaModulo($matricula,
                $cod_disciplina, $aluno['ref_cod_serie'], count($this->array_modulos));
            } else {
              $obj_nota_aluno = new clsPmieducarNotaAluno();
          ...
          $this->pdf->escreve_relativo($media, $esquerda, $this->page_y + 4,
            $espaco_disciplinas, 50, $fonte, $tam_texto + 1, $corTexto, 'center');
          $esquerda += $espaco_disciplinas;
          $this->pdf->linha_relativa($esquerda, $this->page_y, 0, $altura);
        }
        ...
        $this->pdf->escreve_relativo($faltas ? $faltas : '0', $esquerda, $this->page_y + 4,
          $espaco_disciplinas, 50, $fonte, $tam_texto + 1, $corTexto, 'center');
  ...
  function montaDisciplinasGlobalizada($esquerda, $espaco_disciplina)
  {
    ...
    foreach ($this->disciplinas as $disciplina) {
      ...
      $this->pdf->escreve_relativo($disciplina['abreviatura'], $esquerda,
        $this->page_y - $altura, $espaco_disciplinas, 100, $fonte, $tam_texto,
        $corTexto, 'center');

      $this->pdf->escreve_relativo($disciplina['carga_horaria'] . ' hrs',
        $esquerda, $this->page_y + 10 - $altura, $espaco_disciplinas, 50,
        $fonte, $tam_texto - 2, $corTexto, 'center');

      $this->pdf->escreve_relativo('Nota / Conceito', $esquerda, $this->page_y + 3,
        $espaco_disciplinas, 50, $fonte, $tam_texto - 2, $corTexto, 'center');
    ...
    $this->pdf->linha_relativa($esquerda, $this->page_y - $altura, 0, $altura * 2);
    $this->pdf->escreve_relativo('Faltas', $esquerda, $this->page_y, $espaco_disciplinas,
      50, $fonte, $tam_texto, $corTexto, 'center');

    $this->pdf->linha_relativa($esquerda += $espaco_disciplinas,
  ...
  function montaDisciplinasNaoGlobalizada($esquerda, $espaco_disciplina)
  {
    ...
    foreach ($this->disciplinas as $disciplina) {
      ...
      $this->pdf->escreve_relativo($disciplina['abreviatura'], $esquerda,
        $this->page_y - $altura, $espaco_disciplinas, 100, $fonte, $tam_texto, $corTexto, 'center');

      $this->pdf->escreve_relativo($disciplina['carga_horaria'] . ' hrs', $esquerda,
        $this->page_y + 10 - $altura, $espaco_disciplinas, 50, $fonte,
        $tam_texto - 2, $corTexto, 'center');

      $this->pdf->escreve_relativo('Nota / Conceito', $esquerda, $this->page_y + 3,
        $espaco_disciplinas / 2, 50, $fonte, $tam_texto - 2, $corTexto, 'center');

      $this->pdf->escreve_relativo('Falta', $esquerda + $espaco_disciplinas / 2,
        $this->page_y + 3, $espaco_disciplinas / 2, 50, $fonte, $tam_texto - 2,
        $corTexto, 'center');

      $this->pdf->linha_relativa($esquerda, $this->page_y - $altura, 0, $altura * 2);

      $this->pdf->linha_relativa($esquerda + $espaco_disciplinas / 2, $this->page_y, 0, $altura);
