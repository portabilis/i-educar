...
class indice extends clsCadastro
{
    ...
    {
        $this->inputsHelper()->dynamic('ano', ['value' => $this->ano]);
    ...
    public function importaRegistro00($dadosRegistro)
    {
    ...
    $email = $dadosRegistro[25];
        ...
        $convenioPoderPublico,
        $mantenedorEmpresa,
        $mantenedorSindicato,
        $mantenedorOrganizacao,
        $mantenedorInstituicao,
        $mantenedorSistema,
    ...
    public function importaRegistro10($dadosRegistro)
    {
        $inep = $dadosRegistro[2-1];

        $localFuncionamentoArray = [
      '3' => $dadosRegistro[3-1],
      '4' => $dadosRegistro[4-1],
      '5' => $dadosRegistro[5-1],
      '6' => $dadosRegistro[6-1],
      ...
      'proposta_pedagogica' => $dadosRegistro[107-1],
        ...
        $camposEscola['abastecimento_agua'] = [];
        for ($i=1; $i <= 5; $i++) {
        ...
        $camposEscola['abastecimento_energia'] = [];
        for ($i=1; $i <= 4; $i++) {
    ...
    public function importaRegistro20($dadosRegistro)
    {
      ...
      'tipo_mediacao_didatico_pedagogico' => $dadosRegistro[6-1]
        ...
        $camposTurma['atividades_complementares'] = [];
        for ($i=1; $i <= 6; $i++) {
    ...
    public function vinculaDisciplinaTurma($codEscola, $codSerie, $codTurma, $disciplinaEducacenso)
    {
    ...
    public function vinculaDisciplinaEscolaSerie($codDisciplina, $codEscola, $codSerie)
    {
    ...
    public function vinculaDisciplinaSerie($codDisciplina, $codSerie)
    {
        $dataMapper = Portabilis_DataMapper_Utils::getDataMapperFor('componenteCurricular', 'anoEscolar');
        $where = ['componente_curricular_id'=>$codDisciplina, 'ano_escolar_id' => $codSerie];
        $componenteAno = $dataMapper->findAll([], $where);
    ...
    public function getOrCreateComponenteCurricularTurma($codEscola, $codSerie, $codTurma, $codDisciplina)
    {
        $dataMapper = Portabilis_DataMapper_Utils::getDataMapperFor('componenteCurricular', 'turma');
        $where = ['componente_curricular_id'=>$codDisciplina, 'turma_id' => $codTurma];
        $turmas = $dataMapper->findAll([]);
    ...
    public function getOrCreateAreaConhecimento()
    {
        ...
        if (count($areas)) {
            ...
            $codArea = Portabilis_Utils_Database::selectField($sql, [$this->ref_cod_instituicao, 'Migração']);
        }

        return $codArea;
    }

    public function getOrCreateDisciplina($disciplinaEducacenso)
    {
        ...
        $where = ['codigo_educacenso' => $disciplinaEducacenso];

        $disciplinas = $dataMapper->findAll([], $where);
        $codDisciplina = null;
    ...
    public function getOrCreateTurmaTipo()
    {
        $codTurmaTipo= null;
    ...
    public function getOrCreateSerie($etapaEnsinoCenso, $codEscola, $codCurso, $tipoAtendimento)
    {
    ...
    public function getOrCreateCurso($etapaEnsinoCenso, $codEscola, $modalidade, $tipoAtendimento)
    {
    ...
    public function getOrCreateTipoEnsino()
    {
    ...
    public function getOrCreateNivelEnsino($nivel = 'Ano')
    {
    ...
    public function getNivelEnsino($nivel)
    {
    ...
    public function importaRegistro30($dadosRegistro)
    {
        $inepEscola = $dadosRegistro[2-1];
    ...
    public function getOrCreateFuncaoProfessor()
    {
    ...
    public function importaRegistro40($dadosRegistro)
    {
        $inepEscola = $dadosRegistro[2-1];
        $inepServidor = $dadosRegistro[3-1];
        //$codServidor = $dadosRegistro[4-1];
    ...
    public function importaRegistro50($dadosRegistro)
    {
        $inepEscola = $dadosRegistro[2-1];
        $inepServidor = $dadosRegistro[3-1];
        //$codServidor = $dadosRegistro[4-1];
      ...
      'ano_conclusao_curso_superior_3' => $dadosRegistro[22-1],
        ...
        $cursosServidor['curso_formacao_continuada'] = [];
        for ($i=1; $i <= 16; $i++) {
    ...
    public function getOrCreateEscolaridade($escolaridade)
    {
        ...
        if (is_array($escolaridades) && count($escolaridades) > 0) {
            ...
            $escolaridadeToString = [1 => 'Fundamental incompleto',
    ...
    public function importaRegistro51($dadosRegistro)
    {
        $inepEscola = $dadosRegistro[2-1];
        $inepServidor = $dadosRegistro[3-1];
        ...
        Portabilis_Utils_Database::fetchPreparedQuery('DELETE FROM modules.professor_turma_disciplina WHERE professor_turma_id = $1', [ 'params' => [$id]]);
        foreach ($disciplinas as $disciplina) {
            $codDisciplina = $this->getOrCreateDisciplina($disciplina);

            Portabilis_Utils_Database::fetchPreparedQuery('INSERT INTO modules.professor_turma_disciplina VALUES ($1,$2)', [ 'params' =>  [$id, $codDisciplina] ]);
        }
    }

    public function importaRegistro60($dadosRegistro)
    {
        $inepEscola = $dadosRegistro[2-1];
    ...
    public function importaRegistro70($dadosRegistro)
    {
        $inepEscola = $dadosRegistro[2-1];
        $inepAluno = $dadosRegistro[3-1];
    ...
    public function importaRegistro80($dadosRegistro)
    {
        $inepEscola = $dadosRegistro[2-1];
        $inepAluno = $dadosRegistro[3-1];
        $inepTurma = $dadosRegistro[5-1];
        ...
        if (!$codMatricula) {
          ...
          null,
          null,
          $detalheTurma['ref_ref_cod_escola'],
          $detalheTurma['ref_ref_cod_serie'],
          null,
          $this->pessoa_logada,
          $codAluno,
          1,
    ...
    public function createOrUpdateAlunoTransporte($codAluno, $utilizaTransporte, $poderPublicoTransporte)
    {
        $transporte = 0;
    ...
    public function removerFlagUltimaMatricula($alunoId)
    {
    ...
    public function getIdCartorioInep($codigo)
    {
    ...
    public function getIdCursoSuperiorEducacenso($codigo)
    {
    ...
    public function getIdInstituicaoEducacenso($codigo)
    {
    ...
    public function createServidorFuncao($codServidor)
    {
    ...
    public function createServidorEducacenso($codServidor, $inepServidor)
    {
        $sql = 'INSERT INTO modules.educacenso_cod_docente (cod_servidor,cod_docente_inep, fonte, created_at)
                                                  VALUES ($1, $2,\'I\', \'NOW()\')';
        Portabilis_Utils_Database::fetchPreparedQuery($sql, ['params' => [$codServidor, $inepServidor]]);
    }

    public function createServidor($idpesServidor)
    {
        $servidor = new clsPmieducarServidor();
    ...
    public function existeEnturmacao($codTurma, $codMatricula)
    {
        $sql = "SELECT 1
    ...
    public function existeMatricula($serie, $aluno, $ano)
    {
    ...
    public function existeAluno($inep)
    {
    ...
    public function existeServidor($inep)
    {
    ...
    public function getOrCreateRaca($racaEducacenso)
    {
    ...
    public function createOrUpdateRaca($idpes, $racaEducacenso)
    {
    ...
    public function cadastraPessoaFisica($nome, $dataNascimento = null, $sexo = null, $idmun = null, $idpesMae = null, $idpesPai = null, $nis = null, $tipoNacionalidade = null, $paisNacionalidadeCenso = null)
    {
        $idpes = null;
        if (!empty($nome)) {
            $pessoa = new clsPessoa_(
    ...
    public function cadastraEndereco($idpes, $cep, $logradouro, $enderecoNumero, $complemento, $nomeBairro, $ufIbge, $municipioIbge, $distritoIbge, $localizacao)
    {
    ...
    public function getOrCreateLogradouro($logradouro, $idmun)
    {
    ...
    public function getLogradouro($logradouro, $idmun)
    {
        $split = explode(' ', $logradouro, 2);
        $parteLogradouro = isset($split[1]) ? $split[1] : $logradouro;
    ...
    public function getOrCreateDeficiencia($deficienciaEducacenso)
    {
    ...
    public function getDeficiencia($deficienciaEducacenso)
    {
    ...
    public function getCurso($nomeCurso)
    {
    ...
    public function getOrCreateBairro($idmun, $iddis, $nomeBairro, $localizacao)
    {
    ...
    public function getBairro($idmun, $iddis, $nomeBairro)
    {
    ...
    public function getDistritoByMunicipio($idmun)
    {
        $sql = "SELECT iddis
    ...
    public function getDistritoByCodIbge($distritoIbge)
    {
        $sql = "SELECT iddis
              from public.distrito
    ...
    public function getMunicipioByCodIbge($municipioIbge)
    {
        $sql = "SELECT idmun
    ...
    public function getUfByCodIbge($ufIbge)
    {
        $sql = "SELECT sigla_uf
              from public.uf
    ...
    public function getPaisByCodIbge($paisIbge)
    {
        $sql = "SELECT idpais
              from public.pais
    ...
    public function getOrgaoEmissorRgByCodCenso($orgaoEmissorRgCenso)
    {
        $sql = "SELECT idorg_rg
    ...
    public function checkEnderecoPessoa($idpes)
    {
    ...
    public function atualizaCamposEscolaRegistro00(
    $codEscola,
      $cargoGestor,
      $situacao,
      $latitude,
    ...
    public function createAlunoEducacenso($codAluno, $inep)
    {
    ...
    public function createEscolaEducacenso($codEscola, $inep)
    {
    ...
    public function createEscolaAnoLetivo($codEscola, $dataInicioAnoLetivo, $dataFimAnoLetivo)
    {
    ...
    public function getOrCreateModulo()
    {
    ...
    public function createEscola($localizacao, $codEscolaRedeEnsino, $idpesEscola, $nomeEscola, $idpesGestor, $cargoGestor, $emailGestor)
    {
    ...
    public function getOrCreateRedeDeEnsino()
    {
    ...
    public function getRedeDeEnsino()
    {
    ...
    public function existePessoa($cpf)
    {
        $sql = "SELECT idpes
    ...
    public function existeTurma($inep)
    {
    ...
    public function existeEscola($inep)
    {
    ...
    public function isAtividadeComplementar($tipoAtendimento)
    {
        return $tipoAtendimento == 4;
    }

    public function isAtendimentoEspecializado($tipoAtendimento)
    {
