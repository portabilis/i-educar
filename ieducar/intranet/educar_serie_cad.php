...
class indice extends clsCadastro
{
	...
	function Gerar()
	{
		...
		$this->campoCheck('media_especial','M&eacute;dia Especial',$this->media_especial, "", false, "apagaUltimoNota();");
		$this->ultima_nota_define = dbBool($this->ultima_nota_define) ? 'true' : '';
		$this->campoCheck("ultima_nota_define", "Nota do Último Módulo Define Situação", $this->ultima_nota_define, "", false, "apagaMediaEspecial();");
...
//		$this->disciplina_serie = unserialize( urldecode( $this->disciplina_serie ) );
		...
		$this->ultima_nota_define = $this->ultima_nota_define ? "true" : "false";

		if ($this->disciplinas)
		{
			$obj = new clsPmieducarSerie( null, null, $this->pessoa_logada, $this->ref_cod_curso, $this->nm_serie, $this->etapa_curso, $this->concluinte, $this->carga_horaria, null, null, 1, $this->intervalo, $this->idade_inicial, $this->idade_final, $this->media_especial, $this->ultima_nota_define );
...
//		$this->disciplina_serie = unserialize( urldecode( $this->disciplina_serie ) );
		if ($this->disciplinas)
		{
			$obj = new clsPmieducarSerie($this->cod_serie, $this->pessoa_logada, null, $this->ref_cod_curso, $this->nm_serie, $this->etapa_curso, $this->concluinte, $this->carga_horaria, null,null, 1, $this->intervalo, $this->idade_inicial, $this->idade_final, $this->media_especial, $this->ultima_nota_define);
			$editou = $obj->edita();
			if( $editou )
			{
			//-----------------------EDITA DISCIPLINA------------------------//
...
function apagaMediaEspecial() {
	if ($('ultima_nota_define').checked)
		$('media_especial').checked = false;
}

function apagaUltimoNota() {
	if ($('media_especial').checked)
		$('ultima_nota_define').checked = false;
}
