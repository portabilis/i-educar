...
class indice extends clsListagem
{
    public function Gerar()
    {
        ...
        $combo = ['0' => 'Pendente','1' => 'Ativo','2' => 'Ambos' ];

        $this->campoLista('status', 'Status', $combo, $_GET['status'], false, false, false, false, false, true);

        $this->addCabecalhos([  'Nome da a&ccedil;&atilde;o', 'Status']);
        // Paginador
        $limite = 10;
        ...
        if ($Lista_acoes) {
            ...
            foreach ($Lista_acoes as $acao) {
                ...
                $this->addLinhas([" <img src='imagens/noticia.jpg' border=0><a href='acoes_acao_det.php?cod_acao_governo={$acao['cod_acao_governo']}'>{$acao['nm_acao']}</a>",$status]);
            }
        }
