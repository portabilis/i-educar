...
class indice extends clsDetalhe
{
    public function Gerar()
    {
        ...
        $this->addDetalhe(['Data de Criação', date('d/m/Y H:i:s', strtotime(substr($detalhe['data_cadastro'], 0, 19)))  ]);

        $obj = new clsGrupoModerador();
        $lista = $obj->lista(false, $cod_grupo);
        if ($lista) {
            $this->addDetalhe(['<b><i>Moderador(es)</i></b>', '']);
            $i = 1;
            foreach ($lista as $moderadores) {
                $obj = new clsPessoaFisica($moderadores['ref_ref_cod_pessoa_fj']);
                $detalhe = $obj->detalhe();
                $this->addDetalhe(["Moderador $i", $detalhe['nome']]);
                $i++;
            }
        }
        ...
        if ($lista) {
            ...
            foreach ($lista as $mebros) {
                ...
                $detalhe = $obj->detalhe();
                $this->addDetalhe(["Membro $i", $detalhe['nome']]);
                $i++;
            }
        }
