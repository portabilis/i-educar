...
class clsListagem extends clsCampos
{
	 ...
	 *
	 * @param Str (caminho da p�gina) $caminho
	 * @param int (total de Registro da Consulta)$qdt_registros
	 * @param int (quantidade de registro por p�gina)$limite
	 * @param int (posi��o atual do paginador)$link_atual
	 */
...
// 			width='602'
		...
		if ($this->campos)
		{
			...
			*/
			...
			if($this->busca_janela)
			{
				...
				//$retorno .=  "<tr><td style=\"padding-left: 10px; padding-right:10px\" class='formdktd' colspan='2' height='24'><span class='form'><b>Filtros de busca</b></span></td>";
			...
			else
			{
				...
				//$retorno .= "<input type='button' class='botaolistagem' onclick='javascript:limpar();' value='limpar'>";
				$retorno .=  "
							</td>
						</tr>
					</table>
		...
		if (!empty($this->paginador))
		{
			...
			foreach ($this->paginador as $pagina)
			{
				if ($pagina[2])
				{
					switch ($pagina[0])
					{
						case "<<": $retorno .=  "<a href='{$pagina[1]}' class='nvp_paginador'>{$pagina[0]}</a>&nbsp;&nbsp;&nbsp;&nbsp;"; break;
						case "-10": $retorno .=  "<a href='{$pagina[1]}' class='nvp_paginador'>{$pagina[0]}</a>&nbsp;&nbsp;&nbsp;&nbsp;"; break;
						case ">>": $retorno .=  "&nbsp;&nbsp;&nbsp;&nbsp;<a href='{$pagina[1]}' class='nvp_paginador'>{$pagina[0]}</a>"; break;
						case "p10": $retorno .=  "&nbsp;&nbsp;&nbsp;&nbsp;<a href='{$pagina[1]}' class='nvp_paginador'>+10</a>"; break;
						default: $retorno .=  "<a href='{$pagina[1]}' class='nvp_paginador'>{$pagina[0]}</a>&nbsp;";
					}
				}
				else
				{
	 ...
	 *
	 * @param String $msg
	 * @param String $url redirecionar apos 1 segundo
	 */
