...
class clsPmieducarSequenciaCurso
{
	 ...
	 * @return object
	 */
	function clsPmieducarSequenciaCurso( $ref_curso_origem = null, $ref_curso_destino = null, $ref_usuario_exc = null, $ref_usuario_cad = null, $data_cadastro = null, $data_exclusao = null, $ativo = null )
	{
		$db = new clsBanco();
		...
		if( is_numeric( $ref_curso_destino ) )
		{
			...
			{
				$tmp_obj = new clsPmieducarCurso( $ref_curso_destino );
			...
			{
				if( $db->CampoUnico( "SELECT 1 FROM pmieducar.curso WHERE cod_curso = '{$ref_curso_destino}'" ) )
				{
					$this->ref_curso_destino = $ref_curso_destino;
				}
			}
		...
		if( is_numeric( $ref_curso_origem ) )
		{
			if( class_exists( "clsPmieducarCurso" ) )
			{
				$tmp_obj = new clsPmieducarCurso( $ref_curso_origem );
				if( method_exists( $tmp_obj, "existe") )
				{
					if( $tmp_obj->existe() )
					{
			...
			{
				if( $db->CampoUnico( "SELECT 1 FROM pmieducar.curso WHERE cod_curso = '{$ref_curso_origem}'" ) )
				{
					$this->ref_curso_origem = $ref_curso_origem;
				}
			}
	 ...
	 * @return array
	 */
	function lista( $int_ref_curso_origem = null, $int_ref_curso_destino = null, $int_ref_usuario_exc = null, $int_ref_usuario_cad = null, $date_data_cadastro_ini = null, $date_data_cadastro_fim = null, $date_data_exclusao_ini = null, $date_data_exclusao_fim = null, $int_ativo = null )
	{
	...
	function excluir()
	{
		...
		/*
			delete
		$db = new clsBanco();
		$db->Consulta( "DELETE FROM {$this->_tabela} WHERE ref_curso_origem = '{$this->ref_curso_origem}' AND ref_curso_destino = '{$this->ref_curso_destino}'" );
		return true;
		*/
