...
class clsPmieducarFaltaAtrasoCompensado
{
	 ...
	 * @return object
	 */
	function clsPmieducarFaltaAtrasoCompensado( $cod_compensado = null, $ref_cod_escola = null, $ref_ref_cod_instituicao = null, $ref_cod_servidor = null, $ref_usuario_exc = null, $ref_usuario_cad = null, $data_inicio = null, $data_fim = null, $data_cadastro = null, $data_exclusao = null, $ativo = null )
	{
		$db = new clsBanco();
		...
		if( is_numeric( $ref_cod_servidor ) && is_numeric( $ref_ref_cod_instituicao ) )
		{
			...
			{
				$tmp_obj = new clsPmieducarServidor( $ref_cod_servidor, null, null, null, null, null, null, 1,  $ref_ref_cod_instituicao );
				if( method_exists( $tmp_obj, "existe") )
				{
					if( $tmp_obj->existe() )
					{
	...
	function cadastra()
	{
		if( is_numeric( $this->ref_cod_escola ) && is_numeric( $this->ref_ref_cod_instituicao ) && is_numeric( $this->ref_cod_servidor ) && is_numeric( $this->ref_usuario_cad ) && is_string( $this->data_inicio ) && is_string( $this->data_fim ) )
		{
			$db = new clsBanco();
	...
	function excluir()
	{
		...
		/*
			delete
		$db = new clsBanco();
		$db->Consulta( "DELETE FROM {$this->_tabela} WHERE cod_compensado = '{$this->cod_compensado}'" );
		return true;
		*/
	...
	function ServidorHorasCompensadas( $int_ref_cod_servidor = null, $int_ref_cod_escola = null, $int_ref_cod_instituicao )
	{
		if( is_numeric( $int_ref_cod_servidor ) )
		{
			...
			date( "Y-m-d", time );*/
			$db = new clsBanco();
			$db->Consulta("SELECT fac.data_inicio,
								  fac.data_fim
					    	 FROM pmieducar.falta_atraso_compensado fac
					    	WHERE fac.ref_cod_servidor 		  = '{$int_ref_cod_servidor}'
