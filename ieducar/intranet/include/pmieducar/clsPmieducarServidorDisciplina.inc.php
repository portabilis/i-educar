...
class clsPmieducarServidorDisciplina
{
	...
	function cadastra()
	{
		if( is_numeric( $this->ref_cod_disciplina ) && is_numeric( $this->ref_ref_cod_instituicao ) && is_numeric( $this->ref_cod_servidor ) )
		{
			$db = new clsBanco();
	 ...
	 * @return array
	 */
	function lista( $int_ref_cod_disciplina = null, $int_ref_ref_cod_instituicao = null, $int_ref_cod_servidor = null   )
	{
	...
	function excluir()
	{
		...
		/*
			delete
		$db = new clsBanco();
		$db->Consulta( "DELETE FROM {$this->_tabela} WHERE ref_cod_disciplina = '{$this->ref_cod_disciplina}' AND ref_ref_cod_instituicao = '{$this->ref_ref_cod_instituicao}' AND ref_cod_servidor = '{$this->ref_cod_servidor}'" );
		return true;
		*/
