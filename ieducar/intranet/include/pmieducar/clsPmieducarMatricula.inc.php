...
class clsPmieducarMatricula extends Model
{
   ...
   /*  public function pegarTurma()
    {

        $sql = "
        SELECT a.ref_cod_matricula, a.ref_cod_turma, b.ref_ref_cod_serie, c.nm_turma
    FROM pmieducar.matricula_turma AS a

    JOIN pmieducar.matricula AS b

        ON a.ref_cod_matricula= b.cod_matricula

    JOIN pmieducar.turma AS c

        ON a.ref_cod_turma = c.cod_turma
    ...
    if(is_numeric($cod_matricula)){
        $filtros .= "{$whereAnd} ref_cod_matricula = {$cod_matricula}";
        $whereAnd = 'AND ';
    }

    if(is_numeric($ref_ref_cod_serie)){
        $filtros .= "{$whereAnd} ref_ref_cod_serie = {$ref_ref_cod_serie}";
        $whereAnd = 'AND ';
    }

    if(is_string($nm_turma)){
        $filtros .= "{$whereAnd} nm_turma = {$nm_turma}";
        $whereAnd = 'AND ';
    }

    $db = new clsBanco();
    $countCampos = count(explode(',', "a.ref_cod_matricula, b.ref_ref_cod_serie,c.nm_turma"));
    $resultado = [];

    $sql .= $filtros . $this->getOrderby() . $this->getLimite();

    $db->CampoUnico("SELECT COUNT(0) FROM pmieducar.matricula_turma AS a
    ...
    ON a.ref_cod_turma = c.cod_turma
     ...
     $this->_total = $db->Consulta($sql);
        ...
        if ($countCampos > 1) {
            ...
            while ($db->ProximoRegistro()) {
                $tupla = $db->Tupla();
                $resultado[] = $tupla["a.ref_cod_matricula, b.ref_ref_cod_serie,c.nm_turma"];
            }
        }
