...
class clsCampos
{
	...
	function campoAdicionaTab($nome_tab, $desabilitado_tab = false)
	{
		...
		$this->campos['tab_name_'.$this->__id_tab] = array('nome' => $nome_tab,'id'=>$this->__id_tab);
	...
	function campoTabelaInicio($nome , $titulo = "",$arr_campos = array()/*, $arr_obrigatorio = array()*/, $arr_valores = array(array()),$largura = "",$array_valores_lista = array(array()))
	{
	...
	function campoAnexarLista( $nome, $campo, $valor, $default, $valores_ja_incluidos = false, $caminho="", $acao = "", $duplo=false, $descricao="", $descricao2="" )
	{
		$this->campos[$nome] = array( "anexarListaPesquisa", $campo, "", $valor, $default, $acao, $descricao, $caminho, $descricao2, $valores_ja_incluidos );
	}
	...
	function campoBoolLista( $nome, $campo, $default, $val_true = "Sim", $val_false = "Não", $val_undefined = null )
	{
		$valor = array();
		$valor["f"] = $val_false;
		$valor["t"] = $val_true;
...
//	function campoBoolRadio( $nome, $arr_campos = array(), $arr_valores = array() )
//	{
//		$this->campos[$nome] = array("addCampos", $nome, false, $arr_campos, $arr_valores );
//	}
	...
	function campoAdicionar( $nome, $campo, $valor, $default, $acao = "" , $duplo=false, $descricao="", $link, $url_janela, $largura_j, $altura_j, $obrigatorio = false, $redirecionamento="")
	{
		$this->campos[$nome] = array( "adicionar", $campo, $obrigatorio ? "/[^ ]/" : "", $valor, $default, $acao, $descricao, $link, $url_janela, $largura_j, $altura_j, $redirecionamento );
	}

	function campoArquivo( $nome, $campo, $valor, $tamanho, $descricao="", $tr_invisivel = false )
	{
		$this->campos[$nome] = array( "arquivo", $campo, "", $valor, $tamanho, $descricao, "tr_invisivel"=>$tr_invisivel);
		$this->form_enctype = " enctype='multipart/form-data'";
	}

	function campoCep( $nome, $campo, $valor, $obrigatorio = false, $hifen = "-", $descricao = false, $disable = false)
	{
		$arr_componente = array( "cep", $this->__adicionando_tabela ? $nome : $campo, $obrigatorio ? "/([0-9]{5})$hifen([0-9]{3})/" : "*(/([0-9]{5})$hifen([0-9]{3})/)", $valor, 10, (8+@strlen($hifen)), "nnnnn-nnn" , $descricao, ($disable) ? "disabled" : "");
	...
	function campoCheck( $nome, $campo, $valor, $desc = "", $duplo=false, $script = false, $disable = false)
	{
		$arr_componente = array( $duplo ? "checkDuplo": "check", $this->__adicionando_tabela ? $nome : $campo, false, $valor, $desc, $script, ($disable) ? "disabled" : "" );
	...
	{
		$arr_componente = array( "cnpj",$this->__adicionando_tabela ? $nome : $campo, $obrigatorio ? "/[0-9]{2}\.[0-9]{3}\.[0-9]{3}\/[0-9]{4}\-[0-9]{2}/" : "*(/[0-9]{2}\.[0-9]{3}\.[0-9]{3}\/[0-9]{4}\-[0-9]{2}/)", $valor, 20, 18, "nn.nnn.nnn/nnnn-nn" );
	...
	function campoCnpjPesq( $nome, $campo, $valor, $arquivo_pesquisa, $parametros_serializados = false, $obrigatorio = false )
	{
		$this->campos[$nome] = array( "cnpj_pesq", $campo, $obrigatorio ? "/[0-9]{2}\.[0-9]{3}\.[0-9]{3}\/[0-9]{4}\-[0-9]{2}/" : "*(/[0-9]{2}\.[0-9]{3}\.[0-9]{3}\/[0-9]{4}\-[0-9]{2}/)", $valor, 20, 18, "nn.nnn.nnn/nnnn-nn", $arquivo_pesquisa, $parametros_serializados );
	}

	function campoCor( $nome, $campo, $valor, $obrigatorio = false, $descricao="")
	{
		$this->campos[$nome] = array( "cor", $campo, $obrigatorio ? "/[^ ]/" : "", $valor, "cor", "cor", $descricao );
	}

	function campoCpf( $nome, $campo, $valor, $obrigatorio = false, $descricao = false, $disabled = false)
	{

		$arr_componente = array( "cpf", $this->__adicionando_tabela ? $nome : $campo, $obrigatorio ? "/[0-9]{3}\.[0-9]{3}\.[0-9]{3}-[0-9]{2}/" : "", $valor, 16, 14, "nnn.nnn.nnn-nn",$descricao,$disabled);
	...
	function campoIdFederal( $nome, $campo, $valor, $obrigatorio=false, $invisivel=false, $descricao=false)
	{
		$arr_componente= array( "idFederal", $this->__adicionando_tabela ? $nome : $campo, $obrigatorio ?"/[0-9]{3}\.[0-9]{3}\.[0-9]{3}-[0-9]{2}/+/[0-9]{2}\.[0-9]{3}\.[0-9]{3}\/[0-9]{4}\-[0-9]{2}/" :"", $valor, 20, 18, "nnn.nnn.nnn-nn ou nn.nnn.nnn/nnnn-nn", $invisivel ? "disabled" : "" , $descricao);
	...
	function campoData( $nome, $campo, $valor, $obrigatorio = false, $descricao="", $duplo=false, $acao="",$disabled = false )
	{
		$arr_componente = array( $duplo?"dataDupla":"data", $this->__adicionando_tabela ? $nome : $campo, $obrigatorio ? "/(((0[1-9]|[12][0-9])\/(02))|((0[1-9]|[12][0-9]|(30))\/(0[4689]|(11)))|((0[1-9]|[12][0-9]|3[01])\/(0[13578]|(10)|(12))))\/[1-2][0-9]{3}/" : "*/(((0[1-9]|[12][0-9])\/(02))|((0[1-9]|[12][0-9]|(30))\/(0[4689]|(11)))|((0[1-9]|[12][0-9]|3[01])\/(0[13578]|(10)|(12))))\/[1-2][0-9]{3}/", $valor, 9, 10, "dd/mm/aaaa", $descricao, $acao,$disabled);
	...
	function campoDetalhe( $nome, $campo, $valor, $default, $acao = "" , $duplo=false, $descricao="", $link, $url_janela, $largura_j, $altura_j, $obrigatorio = false)
	{
		$this->campos[$nome] = array( "detalhe", $campo, $obrigatorio ? "/[^ ]/" : "", $valor, $default, $acao, $descricao, $link, $url_janela, $largura_j, $altura_j );
	}

	function campoExterno( $nome, $campo, $valor, $tamanhovisivel, $tamanhomaximo, $caminho, $nome_caminho, $obrigatorio = false,  $duplo=false, $descricao="")
	{
		$this->campos[$nome] = array( "textoExterno", $campo, ($obrigatorio ? "/[^ ]/" : ""), $valor, $tamanhovisivel, $tamanhomaximo, $descricao, $caminho, $nome_caminho );
	}

	function campoProcurarAdicionar( $nome, $campo, $valor, $tamanhovisivel, $tamanhomaximo, $caminho, $nome_caminho, $caminho_dest, $nome_caminho_dest ,$obrigatorio = false,  $duplo=false, $descricao="")
	{
		$this->campos[$nome] = array( "adicionarProcurar", $campo, ($obrigatorio ? "/[^ ]/" : ""), $valor, $tamanhovisivel, $tamanhomaximo, $descricao, $caminho, $nome_caminho,$caminho_dest, $nome_caminho_dest );
	}

	function campoFone( $nome, $campo, $valor, $obrigatorio = false)
	{
		$this->campos[$nome] = array( "fone", $campo, $obrigatorio ? "/^\([0-9]{2}\)[0-9]{3}[0-9]?\-[0-9]{4}$/" : "*(/^\([0-9]{2}\)[0-9]{3}[0-9]?\-[0-9]{4}$/)", $valor, 20, 13, "(nn)nnnn-nnnn ou (nn)nnn-nnnn" );
	}

	function campoHora( $nome, $campo, $valor, $obrigatorio = false, $descricao="", $acao="" )
	{
		$arr_componente = array( "hora", $this->__adicionando_tabela ? $nome : $campo, $obrigatorio ? "/[0-9]{2}:[0-9]{2}/" : "*(/[0-9]{2}:[0-9]{2}/)", $valor, 6, 5, "hh:mm", $descricao, $acao );
	...
	function campoLista( $nome, $campo, $valor, $default, $acao = "" , $duplo=false, $descricao="", $complemento="", $desabilitado = false, $obrigatorio = true,$multiple = false )
	{
		$filtro = "";
		if( $obrigatorio )
		{
		...
		$arr_componente = array( $duplo?"listaDupla":"lista", ($this->__adicionando_tabela === true? $nome : $campo), $filtro, $valor, $default, $acao, $descricao, $complemento, $desabilitado ? "disabled='disabled'" : "",$multiple );
	...
	function campoListaDin( $nome, $campo, $valor, $default, $acao = "" , $arquivo,  $campo_destino, $_GET_campos = array(), $complemento="", $desabilitado = false, $obrigatorio = true, $function_extra = "", $duplo=false )
	{
		$filtro = ( $obrigatorio ) ? "/[^ ]/": "";
		$this->campos[$nome] = array( "listaDin", $campo, $filtro, $valor, $default, $acao, "", $campo_destino, $complemento, $desabilitado ? "disabled" : "", $arquivo, $nome, $_GET_campos, $function_extra, $duplo );
	}

	function campoListaAtivarPeso( $nome, $campo, $valor, $default="0", $ativado=false, $colunas="10", $tamanho="10", $default1="")
	{
		$this->campos[$nome] = array( "listaativarpeso", $campo, "/[^ ]/", $valor, $default, $ativado, "", $colunas, $tamanho, $default1 );
	}

	function campoListaAtivarPesoSelect( $nome, $campo, $valor, $default="0", $ativado=false,$valor2)
	{
		$this->campos[$nome] = array( "listaativarpesoselect", $campo, "/[^ ]/", $valor, $default, $ativado, $valor2);
	}

	function campoListaPesq( $nome, $campo, $valor, $default, $caminho="", $acao = "" , $duplo=false, $descricao="", $descricao2="", $flag=null, $pag_cadastro = null, $disabled = "", $div = false, $serializedcampos = false, $obrigatorio = false )
	{
		$this->campos[$nome] = array( "listapesquisa", $campo, $obrigatorio ? "/[^ ]/": "", $valor, $default, $acao, $descricao, $caminho, $descricao2, $flag, $pag_cadastro, $disabled, $div, $serializedcampos, $obrigatorio );
	}

	function campoMemo( $nome, $campo, $valor, $colunas, $linhas, $obrigatorio = false, $descricao="", $conta="", $duplo = false, $script = false, $evento = "onclick", $disabled = false  )
	{
		$this->campos[$nome] = array( "memo", $campo, $obrigatorio ? "/[^ ]/" : "", $valor, $colunas, $linhas, $descricao, $conta, $duplo, $evento, $script, $disabled );
	}

	function campoNumero( $nome, $campo, $valor, $tamanhovisivel, $tamanhomaximo, $obrigatorio = false, $descricao="", $descricao2 = "", $script = false, $evento = false, $duplo = false, $disabled = false  )
	{
		$arr_componente = array( $duplo ? "textoDuplo" : "texto", $this->__adicionando_tabela ? $nome : $campo, $obrigatorio ? "/^[0-9]+$/" : "*(/^[0-9]+$/)", $valor, $tamanhovisivel, $tamanhomaximo, $descricao ? $descricao : "somente números", $descricao2, $script, $evento, $disabled );
	...
	function campoMonetario( $nome, $campo, $valor, $tamanhovisivel, $tamanhomaximo, $obrigatorio = false, $descricao="", $script = "", $evento = "onChange", $disabled=false, $show_sub = true,$descricao2="", $duplo = false )
	{
		$arr_componente = array($duplo ? "monetarioDuplo" : "monetario", $this->__adicionando_tabela ? $nome : $campo, $obrigatorio ? "/^[0-9.,]+$/" : "*(/^[0-9.,]+$/)", $valor, $tamanhovisivel, $tamanhomaximo, $descricao, $script, $evento, $disabled , $show_sub,$descricao2);
	...
	{
		$arr_componente = array( "oculto", $nome, "", $valor );
	...
	{
		$this->campos[$nome] = array( "radio", $campo, "", $valor, $default, $acao, $descricao );
	}

	function campoRotulo( $nome, $campo, $valor, $duplo = false, $descricao="", $separador = ":" )
	{
		$arr_componente = array( $duplo ? "rotuloDuplo":"rotulo", $this->__adicionando_tabela ? $nome : $campo, "", $valor, 6=>$descricao, "separador"=>$separador );
	...
	{
		$this->campos[$nome] = array( "senha", $campo, $obrigatorio ? "/[^ ]/" : "", $valor, 10, 100, $descricao );
	}

	function campoTexto( $nome, $campo, $valor, $tamanhovisivel, $tamanhomaximo, $obrigatorio = false, $expressao = false, $duplo=false, $descricao="", $descricao2 = "", $script="", $evento = "onKeyUp", $disabled=false )
	{

		$arr_componente = array( $duplo ? "textoDuplo":"texto", $this->__adicionando_tabela ? $nome : $campo, $expressao ? $expressao :( $obrigatorio ? "/[^ ]/" : ""), $valor, $tamanhovisivel, $tamanhomaximo, $descricao, $descricao2, $script, $evento, $disabled );
	...
	function campoEmail( $nome, $campo, $valor, $tamanhovisivel, $tamanhomaximo, $obrigatorio = false, $expressao = false, $duplo=false, $descricao="", $descricao2 = "", $script="")
	{
		$this->campos[$nome] = array( $duplo ? "emailDuplo":"email", $campo, $expressao ? $expressao :( $obrigatorio ? "/^[a-z_\-\.0-9]+$/" : ""), $valor, $tamanhovisivel, $tamanhomaximo, $descricao, $descricao2, $script );
	}

	function campoSuggest( $nome, $campo, $valor, $obrigatorio = false, $expressao = false, $duplo=false, $descricao="", $arquivo = "xml_pessoas.php" )
	{
		$this->campos[$nome] = array( "suggest", $campo, $expressao ? $expressao :( $obrigatorio ? "/[^ ]/" : ""), $valor, $descricao, $descricao2 );
	}

	function campoTextoPesquisa( $nome, $campo, $valor, $tamanhovisivel, $tamanhomaximo, $obrigatorio = false, $caminho = "", $expressao = false, $duplo=false, $descricao="", $descricao2 = "", $serializedcampos = null, $disabled = false, $script="", $evento = "onChange")
	{
		$arr_componente = array( $duplo ? "textoPesquisaDuplo":"textoPesquisa", $this->__adicionando_tabela ? $nome : $campo, $expressao ? $expressao :( $obrigatorio ? "/[^ ]/" : ""), $valor, $tamanhovisivel, $tamanhomaximo, $descricao, $descricao2, $caminho, $serializedcampos, $disabled, $script, $evento );
	...
	function campoTextoInv( $nome, $campo, $valor, $tamanhovisivel, $tamanhomaximo, $obrigatorio = false, $expressao = false, $duplo=false, $descricao="",$descricao2="", $script = "", $evento = "onKeyUp",$name = "")
	{
		$name = $name ? $name : $nome;
		$this->campos[$nome] = array( $duplo ? "textoDuploInv":"textoInv", $campo, $expressao ? $expressao :( $obrigatorio ? "/[^ ]/" : ""), $valor, $tamanhovisivel, $tamanhomaximo, $descricao,$descricao2, $script, $evento,$name );
	}

	function campoTheEditor	( $nome, $campo, $valor, $largura, $altura )
	{
		$this->campos[$nome] = array( "theEditor", $campo, $valor, $largura, $altura );
	}
	...
	{
		$this->campos["linha_preta".$this->num_espaco] = array( "linha_preta", "cor"=>$cor, "altura"=>$altura, "", "", "", "", "" );
		$this->num_espaco++;
	}
	...
	function MakeCampos( $array_campos = null, $adicionador_indice = null, $todos_inline = false, $todos_disabled = false, $junta_linhas = false, $start_md = null  )
	{
		...
		$adicionador_total_valores = 5;	// marca quantos valores foram passados para o prenchimento das repeticoes

				$javascript .= "
								function tabela(name,counter)
								{
									//seta variavel para nao reordenar id dos campos
									...
									this.addRow = function ()
													{
														...
														var This = this;
														//if(This.id == 0)
															//setVisibility(document.getElementById('tr_' + This.nome + '_cab'),true);
														...
														//var img = campo.getElementsByTagName('IMG')[0];
														//var link = campo.getElementsByTagName('A')[0];
														...
														for(var co=0;co<campos_oculto.length;co++)
														{
															...
															//campos_oculto[co].name = /[a-zA-Z_-]*/.exec(campos_oculto[co].name) + '[]';
									...
									this.removeRow = function(row)
													{
														...
														if(This.id == 0)
															This.addRow();
															//setVisibility(document.getElementById('tr_' + This.nome + '_cab'),false);
													...
													for(var ct=2;ct<trs.length;ct++)
													{
														...
														for(var c=0;c<trs[ct].cells.length;c++)
														{
															...
															for(var inp = 0 ;inp<campos.length;inp++)
															{
																...
																//campos[inp].setAttribute( \"name\",nome_inp + '[]');
		...
		while (list( $nome, $componente ) = each( $arr_campos ))
		{
			...
			if(ereg("^(tabbed_add_[0-9]+)",$nome) === 1)
			{
				...
				$rand = rand();
				// $retorno .= "<!-- INICIO TABELA 1 --><div style='width: 300'><table cellpadding=\"0\" cellspacing=\"0\" border=\"0\" width=\"350\" height=\"250\" id=\"tabela_principal\" align=center>
			...
			if(ereg("^(tab_add_[0-9]+)",$nome) === 1)
			{
				...
				//echo '<pre>';print_r($campos_oculto);die;
				$classe = $md ? 'formlttd' : 'formmdtd';
				$md = $md ? false : true;
				...
				foreach ($cabecalho as $key => $cab)
				{
					...
					//$retorno.= "<td class='formmdtd' id='td_$cab' align='center'><span class='form'>$cab</span>{$obrigatorio}{$explicacao}</td>";
...
//			link.onclick = function() {This.removeRow(this);};
//			img.setAttribute( \"border\",\"0\");
//			img.src = 'imagens/banco_imagens/excluirrr.gif';

				//$click = "$nome_add.addRow();";
				$click = "$nome_add.removeRow(this);";

				//$img = "<img src=\"imagens/banco_imagens/incluir.gif\" border=\"0\" alt=\"incluir\" />";
				...
				foreach ($valores as $key2 => $valor)
				{
					...
					foreach ($componente as $key => $campo_)
					{
						...
						$center = (strtolower($campo_[0]) == 'rotulo' || strtolower($campo_[0]) == 'check' || $largura) ? 'align="center"' : "";
						...
						switch (strtolower($campo_[0]))
						{
							...
							case 'lista':
								...
								$lista = (sizeof($lista)) ? $lista : $campo_[3];
					...
					if(is_array($campos_oculto))
					{
						foreach ($campos_oculto as $key_oculto => $campo_oculto)
						{
							...
							$campo_oculto[3] = $campo_oculto[3] ? $campo_oculto[3] : $valor[$key_oculto];
				...
				$click = "$nome_add.addRow();";
				//$img = "<img src=\"imagens/nvp_bot_adiciona.gif\" border=\"0\" alt=\"incluir\" />";
			...
			{
				// adiciona o campo no bloco de repeticao
			...
			{
//				$retorno .=  "<tr><td height='1' colspan='2'></td></tr>";
				$retorno .=  "<tr><td  style='padding:0px;background-color:{$componente["cor"]};' colspan='2' height='{$componente["altura"]}'></td></tr>";
//				$retorno .=  "<tr><td height='1'></td></tr>";
				continue;
			}
			else if ($componente[0] == "espaco")
			{

				$retorno .=  "<tr><td colspan='2'><hr></td></tr>";
				continue;
			}
			else if ( $componente[0] == "bloco_adicionador" )
			{
				if( $componente[1] == "begin" )
				{
					// flag de que estamos no meio de um bloco de adicao
					$this->__adicionando = true;
					// se for a primeira vez que cai aqui define o indice como zero,
					// nas vezes subsequentes (chamadas por recursividade) o indice será
					// passado na chamada da funcao
					...
					// adiciona o campo begin no inicio do bloco de repeticao
					$adicionador_campos_repeticao[$nome] = $componente;
				}
				else
				{
					// desmarca da flag de bloco de adicao
					$this->__adicionando = false;
					// adiciona o campo end no fim do bloco de repeticao
					$adicionador_campos_repeticao[$nome] = $componente;
					// ainda nao repetiu o bloco por tempo suficiente
					$adicionador_indice++;
					if( $adicionador_indice < $adicionador_total_valores )
					{
						$retorno .= $this->MakeCampos( $adicionador_campos_repeticao, $adicionador_indice, false, true, true, ! $md );
					}
				}
			}
			else if ( $componente[0] != "oculto" )
			{
				...
				{
					if($componente[10] && ($componente[0] == "textoDuploInv" || $componente[0]  =="textoInv" ))
				...
				elseif (!$foiDuplo )
				{
					if ($campo == "-:")
					{
						if (empty( $componente[3] )) $componente[3] = renderpixel();
						$explicacao = ( $componente[6] ) ? "<br><sub style='vertical-align:top;'>{$componente[6]}</sub>": "";
					...
					{	if($componente[10] && ($componente[0] == "textoDuploInv" || $componente[0]  =="textoInv" ))
						...
						$style = ($componente["tr_invisivel"]) ? "style='visibility:collapse'" : "";
						$explicacao = ( $componente[6] ) ? "<br><sub style='vertical-align:top;'>{$componente[6]}</sub>": "";
						$retorno .=  "<tr id='tr_$nome' {$name} $style><td class='$classe' valign='top'><span class='form'>$campo</span>{$obrigatorio}{$explicacao}</td><td class='$classe' valign='top'><span class='form'>\n";
					}
				}
				...
				switch ( $tipo )
				{
					...
					case "anexarListaPesquisa":
					{
						...
						{
							// existem itens ja incluidos, monta a lista com eles (com o botao remover ao lado)
							foreach ( $componente[9] AS $ja_incluido )
							{
								...
								if( $componente[7] )
								{
									// eh um adicionador, entao vamos inserir os ja incluidos na lista padrao
									$opcoes[$cod] = $label;
								}
								reset( $opcoes );
						...
						// array( "anexarListaPesquisa", $campo, "/[^ ]/", $valor, $default, $acao, $descricao, $caminho, $descricao2 );
						$retorno .=  "<tr><td><select onchange=\"{$componente[5]}\"  class='{$class}' name='{$nome}[]' id='{$nome}_{$inicio}'>";
						$javascript .= "
						function remove_{$nome}( id )
						{
							...
							for( i = 0; i < id; i++ )
							{
								if( itens_{$nome}[i] >= id ){break;}
								if( itens_{$nome}[i] != null ){posicao++;}
							}
							itens_{$nome}[id] = null;
						...
						{
							$retorno .=  "<option id=\"{$nome}_{$inicio}_".urlencode($chave)."\" value=\"".urlencode($chave)."\"";
							//$javascript .= "strConteudo += '<option id=\"{$nome}_' + id + '_".urlencode($chave)."\" value=\"".urlencode($chave)."\"";
							...
							{
								$retorno .=  " selected";
								$javascript .= "sel.options[novoIndice].selected = true;";
							}
							$retorno .=  ">$texto</option>";
							$javascript .=  "
							novoIndice++;
						...
						{
							$retorno .= "<img src=\"imagens/lupa.png\" alt=\"Pesquisa\" border=\"0\" onclick=\"pesquisa_valores_popless('{$componente[7]}', '{$nome}_{$inicio}')\">";
							//$javascript .= "strConteudo += '<img src=\"imagens/lupa.png\" alt=\"Pesquisa\" border=\"0\" onclick=\"pesquisa_valores_popless( \'{$componente[7]}\', \'{$nome}_' + id + '\' )\">';\n";
						}
//						$javascript .= "strConteudo += '</td><td id=\"td_adicionador_{$nome}_' + id + '\" style=\"padding-left:30px;\"><input type=\"button\" name=\"adicionador_{$nome}_' + id + '\" id=\"adicionador_{$nome}_' + id + '\" value=\"[+] Adicionar teste 2\" onclick=\"duplica_{$nome}();\" class=\"botaolistagem\"></td>';\n";
//						$javascript .= "strConteudo += '</td><td id=\"td_adicionador_{$nome}_' + id + '\" style=\"padding-left:30px;\"><img name=\"adicionador_{$nome}_' + id + '\" id=\"adicionador_{$nome}_' + id + '\" onclick=\"duplica_{$nome}();\" src=\"imagens/banco_imagens/entrada2.gif\" title=\"Incluir\" border=0></td>';\n";
//						$javascript .= "tabela = document.getElementById( 'anexador_{$nome}' );\nlinha = tabela.insertRow( tabela.rows.length );\ntrocaConteudo( linha, strConteudo );\nfor( i = id - 1; i > -1; i-- ){\nantecessor = document.getElementById( 'td_adicionador_{$nome}_' + itens_{$nome}[i] );\nbotaoRemover = '<input type=\"button\" name=\"adicionador_{$nome}_' + i + '\" id=\"adicionador_{$nome}_' + i + '\" value=\"[-] Remover removteste\" onclick=\"remove_{$nome}(' + i + ');\" class=\"botaolistagem\">';\nif( typeof antecessor == 'object' ){\ntrocaConteudo( antecessor, botaoRemover );\nbreak;\n}\n}\nitens_{$nome}[id] = id;\n}";
...
//						$retorno .= " {$componente[8]}</td><td id=\"td_adicionador_{$nome}_{$inicio}\" style=\"padding-left:30px;\"><input type=\"button\" name=\"adicionador_{$nome}_{$inicio}\" id=\"adicionador_{$nome}_{$inicio}\" value=\"[+] Adicionar teste\" onclick=\"duplica_{$nome}();\" class=\"botaolistagem\"></td></tr></table><script type=\"text/javascript\">$javascript</script>";
...
//							$retorno .= "<img src=\"imagens/lupa.png\" alt=\"Pesquisa\" border=\"0\" onclick=\"pesquisa_valores_f('{$componente[8]}', '{$nome}')\"> $componente[7]";
						}
						break;
					}
					case "textoInv" :
					...
					case "textoExterno" :
					{
						//"textoExterno", $campo, $obrigatorio, $valor, $tamanhovisivel, $tamanhomaximo, $descricao, $caminho, $nome_caminho
...
//						$arr_campos["linha_{$nome}"] = array("addForm", $nome, $arr_campos );
						...
						if($arr_campos)
						{
							foreach ($arr_campos as $campo => $atributo)
							{
								$atributo["quebra"] = !isset($atributo["quebra"])? true : $atributo["quebra"];
								//echo $atributo["quebra"];
								$arr_tipos[$campo] = $atributo['type'];
								//$quebra = "<br>";//$atributo["quebra"] == true  ? "<br>" : "";
								if( !empty($atributo['campo']) )
									$atributo['campo'] = "<br>{$atributo['campo']}:<br>";
								else
									$atributo['campo'] = "";

								$disabled = !empty($atributo['disabled']) ? " disabled = true " : "";

//								$atributo['campo'] = !empty($atributo['campo']) ? "{$atributo['campo']}:<br>" : " ";
								...
								if($atributo)
								{
									foreach ($atributo as $formId => $valor)
									{
										...
										//$valores .= "document.getElementById( \"{$id}\"+cont{$nome} ).disabled = \"true\"; \n ";
									}

								}
								$inicio .= "{$disabled} >";
								$quebra = $atributo["quebra"] == true || !isset($atributo["quebra"]) ? "<br>" : "";
...
//									$chave = reset($val);
					...
					//				echo "<b>$valor_chave , $disabled</b><br>";
									$disabled = $disabled=="_1" ? "disabled" : "";
									foreach ($arr_tipos as $id => $type)
									{
										...
										elseif($type != 'button')
										{
											if($type == "hidden")
											{
												$valor_chave = (!$valor_chave) ? $chave : $valor_chave;
												$disabled = "";
											}
						...
						$inicio .= "<img hspace='5' src='imagens/banco_imagens/incluir.gif' onclick='adiciona_{$nome}();'><br>
			<br>
			" ;
						$retorno .= "<script type=\"text/javascript\">var cont{$nome}=1;
					...
					//$arr_campos[$nome] = array( "listaativarpeso", $campo, "/[^ ]/", $valor, $default, $ativado, "", $colunas, $tamanho );
					...
					case "listapesquisa" :
					{
						////( "listapesquisa", $campo, "/[^ ]/", $valor, $default, $acao, $descricao, $caminho )
						$class = ($componente[14])?"obrigatorio":"geral";
...
width:500,
	...
	{
		///*******setar foco do campo
		//$onload = ($this->campoFoco != false) ? "if(document.body.onload)document.body.onload=setFocus(\"{$this->campoFoco}\");" : "if(document.body.onload)document.body.onload=setFocus();" ;
		...
		$ret .=  "document.".$this->__nome.".".$this->executa_submete;
		$ret .=  "document.$this->__nome.submit();
				}
			}
	...
	function getCampoTexto($nome,$id = "",$valor = "",$tamanhovisivel = "",$tamanhomaximo = "",$evento = "",$disabled = "",$descricao = "",$class ="",$descricao = "")
	{
		$id = $id ? $id : $nome;
		...
		$tamanhomaximo = $tamanhomaximo ? "maxlength=\"{$tamanhomaximo}\"" : "";

		$tamanhovisivel = $tamanhovisivel ? "size=\"{$tamanhovisivel}\"" : "";
	...
	function getCampoLista($nome,$id = "",$acao = "",$valor,$default,$complemento = "",$desabilitado = false,$class,$multiple = false)
	{
		$id = $id ? $id : $nome;
	...
	function getCampoMonetario($nome,$id="", $valor = "",$tamanhovisivel,$tamanhomaximo,$disabled = false, $descricao = "", $descricao2 = "", $class, $evento = "onChange",$script = "")
	{
		$id = $id ? $id : $nome;
	...
	function getCampoHora($nome,$id="",$valor="",$class,$tamanhovisivel,$tamanhomaximo,$acao="",$descricao="")
	{
		$id = $id ? $id : $nome;
	...
	function getCampoIdFederal($nome,$id = "", $valor, $class,$tamanhovisivel,$tamanhomaximo,$disabled = false)
	{
		$id = $id ? $id : $nome;
	...
	function getCampoCep($nome,$id = "", $valor, $class,$tamanhovisivel,$tamanhomaximo,$disabled = false,$descricao = '')
	{
	...
	function getCampoTextoPesquisa($nome,$id = "", $valor, $class,$tamanhovisivel,$tamanhomaximo,$disabled = false,$caminho,$campos_serializados = null,$descricao = null, $script = null, $evento = null)
	{
