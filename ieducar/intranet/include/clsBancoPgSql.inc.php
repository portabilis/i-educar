...
abstract class clsBancoSQL_
{
  ...
  public function Consulta($consulta, $reescrever = true)
  {
    ...
    if ($reescrever) {
      ...
      $this->strStringSQL = preg_replace( "%LIMIT[ ]{0,3}([0-9]+)[ ]{0,3},[ ]{0,3}([0-9]+)%", "LIMIT $2 OFFSET $1", $this->strStringSQL );
      ...
      $this->strStringSQL = preg_replace( "%(YEAR|MONTH|DAY)[(][ ]{0,3}(([a-z]|_|[0-9])+)[ ]{0,3}[)]%", "EXTRACT( $1 FROM $2 )", $this->strStringSQL );
      // Remove os ORDER BY das querys COUNT()
      // Altera os LIKE para ILIKE (ignore case)
      $this->strStringSQL = preg_replace("% LIKE %", " ILIKE ", $this->strStringSQL);

      $this->strStringSQL = preg_replace("%([a-z_0-9.]+) +ILIKE +'([^']+)'%", "to_ascii($1) ILIKE to_ascii('$2')", $this->strStringSQL);

      $this->strStringSQL = preg_replace("%fcn_upper_nrm%", "to_ascii", $this->strStringSQL);
    }
