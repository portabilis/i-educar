...
* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * */
    ...
    $linhas[0] = [ 1500000, 1422566, 120000, 1422566, 1110000, 900000, 890000, 20 ];
    $legenda = [ 'pontuaçao', 'asteroides', 'naves', 'xxx', 'yyyy' ];
    ...
    for ($i = 0; $i < count($coordenadas); $i++) {
        ...
        for ($j = 0; $j < count($coordenadas[$i]) - 1; $j++) {
            ...
            drawdot([ $x_ini, $y_ini, $x_end, $y_end ], $i, $coordenadas[$i][$j]);
        }
        drawdot([ $x_end, $y_end, $width - 1, $y_end ], $i, $coordenadas[$i][$j]);
    }
    imagepng($im);
    imagedestroy($im);
