...
class indice extends clsDetalhe
{
    public function Gerar()
    {
        ...
        if ($strVersoes) {
            $this->addDetalhe([ 'Vers&otilde;es Anteriores', $strVersoes ]);
        }

        if (! $visivel) {
            $this->addDetalhe([ 'Oculto' , '<b>Este edital esta oculto</b>' ]);
        } else {
            $this->addDetalhe([ 'Visivel' , 'Este edital esta visivel' ]);
        }

        $motivo_alteracao = str_replace("\n", '<br>', $motivo_alteracao);
        $this->addDetalhe(['Motivo da altea&ccedil;&atilde;o', $motivo_alteracao ]);

        $this->addDetalhe(['Versão do Edital', $versao ]);
        $this->addDetalhe(['Data da alteração', date('d/m/Y H:i', strtotime(substr($data_hora, 0, 19))) ]);
        $objPessoa = new clsPessoaFisica();
        $resp_nome = $objPessoa->queryRapida($ref_pessoa, 'nome');
        $this->addDetalhe([ 'Responsável', $resp_nome[0] ]);
        ...
        $this->addDetalhe(['Tipo de Arquivo', "<img src=\"$imagem\"> $extensao" ]);
        $this->addDetalhe(['Arquivo', "<a href=\"{$arquivo}\">{$arquivo}</a>" ]);
        ...
        if ($db->ProximoRegistro()) {
            ...
            $this->addDetalhe(['Numero da Licitação',$numero]);
            $this->addDetalhe(['Modalidade',$nm_modalidade]);
            $this->addDetalhe(['Data da Licitação', date('d/m/Y', strtotime(substr($data, 0, 19))) ]);
        }
