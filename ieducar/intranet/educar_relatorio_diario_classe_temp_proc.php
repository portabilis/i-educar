...
class indice extends clsCadastro
{
  ...
  function renderHTML()
  {
    ...
    if ($det_curso['falta_ch_globalizada']) {
      ...
      $consulta = sprintf('SELECT padrao_ano_escolar FROM pmieducar.curso WHERE cod_curso = %d',
        $this->ref_cod_curso);
      ...
      $this->pdf = new clsPDF('Diário de Classe - ' . $this->ano,
        sprintf('Diário de Classe - %s até %s de %d', $data[0], $data[1], $this->ano),
        'A4', '', FALSE, FALSE);
      ...
      if (!$this->em_branco) {
        ...
        $lista_matricula = $obj_matricula_turma->lista(NULL, $this->ref_cod_turma,
          ...
          NULL, NULL, array(1,2,3), NULL, NULL, $this->ano, NULL, TRUE, NULL, NULL, TRUE);
      }
      ...
      if ($lista_matricula || $this->em_branco) {
        ...
        foreach ($lista_matricula as $matricula) {
          ...
          $this->pdf->escreve_relativo(sprintf('%02d', $num), 757, $this->page_y + 4,
            30, 30, $fonte, 7, $corTexto, 'left');
    ...
    {
      // Carga horária não globalizada gera uma página por disciplina
      ...
      if ($lst_turma_disc) {
        ...
        $this->pdf = new clsPDF('Diário de Classe - ' . $this->ano,
          sprintf('Diário de Classe - %s até %s  de %d', $this->data_ini, $this->data_fim, $this->ano),
          'A4', '', FALSE, FALSE
        );
          ...
          if ($lista_quadro_horarios) {
            for ($mes_ = $meses[0]; $mes_ <= $meses[1]; $mes_++) {
              ...
              foreach ($lista_quadro_horarios as $dia_semana) {
                ...
                $total_dias_semanas += $this->getDiasSemanaMes($dia, $mes_,
                  $this->ano,$dia_semana,$mes_final);
              }
            }
          }
          else {
  ...
  public function addCabecalho()
  {
    ...
    // Data
    $this->pdf->escreve_relativo(sprintf("%s até %s de %d", $this->data_ini,
      $this->data_fim, $this->ano), 45, 100, 782, 80, $fonte, 10, $corTexto, 'center');
  ...
  function rodape()
  {
    ...
    $this->pdf->escreve_relativo('Data: ' . $dataAtual, 36, 795, 100, 50,
      $fonte, 7, $corTexto, 'left');
  ...
  function getNumeroDiasMes($dia, $mes, $ano, $mes_final = FALSE)
  {
    ...
    for ($day = $dia; $day <= $last_day_of_month; $day++) {
      ...
      if (($dia_semana_corrente != 1 &&  $dia_semana_corrente != 7) &&
          (array_search($day, $dias_nao_letivo) === FALSE)
      ) {
        $numero_dias++;
      }
    }
  ...
  function getDiasSemanaMes($dia, $mes, $ano, $dia_semana, $mes_final = FALSE)
  {
    ...
    for($day = $dia; $day <= $last_day_of_month; $day++) {
      ...
      $data_atual = sprintf('%d/%d/%d', $day, $mes, $ano);
      $data_final = sprintf('%s/%d', $this->data_fim);
