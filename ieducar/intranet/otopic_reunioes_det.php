...
class indice extends clsDetalhe
{
    public function Gerar()
    {
        ...
        */
        ...
        $this->addDetalhe(['Descrição', $detalhe['descricao']]);

        $this->addDetalhe(['Data Inicio', date('d/m/Y H:i', strtotime(substr($detalhe['data_inicio_marcado'], 0, 19)))  ]);

        $this->addDetalhe(['Data Fim', date('d/m/Y H:i', strtotime(substr($detalhe['data_fim_marcado'], 0, 19)))  ]);
        ...
        if ($detalhe['data_inicio_real'] && !$detalhe['data_fim_real']) {
            ...
            $this->addDetalhe(['Tempo de Duração', '<div id=\'tempo\'></div>'  ]);
        ...
        if ($detalhe['data_inicio_real']) {
            $this->addDetalhe(['Data Inicio Real', date('d/m/Y H:i', strtotime(substr($detalhe['data_inicio_real'], 0, 19)))  ]);
        }
        if ($detalhe['data_fim_real']) {
            $data_inicial =   strtotime(substr($detalhe['data_inicio_real'], 0, 19));
            $data_final = strtotime(substr($detalhe['data_fim_real'], 0, 19));
            $this->addDetalhe(['Data Fim Real', date('d/m/Y H:i', strtotime(substr($detalhe['data_fim_real'], 0, 19)))  ]);

            $this->addDetalhe(['Tempo de Duração', '<div id=\'tempo\'>'.difTempo($data_inicial, $data_final.'</div>')  ]);
        }
        $this->addDetalhe(['Notificado por e-mail', ($detalhe['email_enviado']) ? 'Sim' : 'Não' ]);

        $this->addDetalhe(['<b><i>Tópicos Relacionados</i></b>', '' ]);
        ...
        foreach ($lista as $topicos) {
            ...
            $this->addDetalhe(['Tópicos', $assunto]);
        }
        ...
        if ($detalhe['data_inicio_real'] && !$detalhe['data_fim_real'] & $detalhe_moderador && $detalhe_moderador['ativo'] == 1) {
            ...
            */
            ...
            $numero = 1;
            if ($lista) {
                $this->addDetalhe(['<b><i>Membro(s)</i></b>', '']);
                foreach ($lista as $moderadores) {
                    ...
                    if ($detalhe_participante['data_saida'] || !$detalhe_participante) {
                        $this->addDetalhe([$nome,"<div id='$numero'><a href='#' onclick='move_pessoa_reuniao({$moderadores['ref_ref_cod_pessoa_fj']},1,$cod_reuniao,$cod_grupo,$numero)'><img src='imagens/nvp_bot_entra_reuniao.gif' border='0'></a></div>"]);
                    } else {
                        $this->addDetalhe([$nome,"<div id='$numero'><a href='#' onclick='move_pessoa_reuniao({$moderadores['ref_ref_cod_pessoa_fj']},2,$cod_reuniao,$cod_grupo,$numero)'><img src='imagens/nvp_bot_sai_reuniao.gif' border='0'></a></div>"]);
                    }
                    $numero++;
                }
            }
            ...
            if ($lista) {
                foreach ($lista as $mebros) {
                    ...
                    $detalhe_participante = $obj->detalhe();
                    if ($detalhe_participante['data_saida'] || !$detalhe_participante) {
                        $this->addDetalhe([$nome,"<div id='$numero'><a href='#' onclick='move_pessoa_reuniao({$mebros['ref_idpes']},1,$cod_reuniao,$cod_grupo,$numero)'><img src='imagens/nvp_bot_entra_reuniao.gif' border='0'></a></div>"]);
                    } else {
                        $this->addDetalhe([$nome,"<div id='$numero'><a href='#' onclick='move_pessoa_reuniao({$mebros['ref_idpes']},2,$cod_reuniao,$cod_grupo,$numero)'><img src='imagens/nvp_bot_sai_reuniao.gif' border='0'></a></div>"]);
                    }
                    $numero++;
                }
            }
        ...
        if (!$detalhe['data_inicio_real'] && $detalhe_moderador && $detalhe_moderador['ativo'] == 1) {
            $this->array_botao = ['Iniciar Reunião', 'Imprimir (Jato)', 'Imprimir (Laser)'];
            ...
            $this->array_botao = ['Finalizar Reunião', 'Imprimir (Jato)', 'Imprimir (Laser)'];
            $this->array_botao_url = ["otopic_reuniao_finaliza.php?cod_reuniao=$cod_reuniao&cod_grupo=$cod_grupo", "$link", "$linkCor"];
        } elseif ($detalhe['data_inicio_real'] && $detalhe['data_fim_real']) {
            $this->array_botao = ['Imprimir (Jato)', 'Imprimir (Laser)'];
            $this->array_botao_url = ["$link", "$linkCor"];
        }
        $this->largura = '100%';
