  ...
  -- Alterando função para que seja utilizada sequence corretamente, pois método utilizada pode estar
  -- estar afetando o auto increment
  --
    ...
    where mat.cod_matricula = old_matricula_id)) THEN
      ...
      IF ((SELECT id FROM modules.regra_avaliacao rg
        ...
        IF (v_tipo_nota >0) THEN            
          ...
          v_nota_id := (SELECT id FROM modules.nota_aluno WHERE matricula_id = new_matricula_id);
        ...
        IF (v_tipo_falta = 1) THEN
            ...
            v_falta_id = (SELECT id FROM modules.falta_aluno WHERE matricula_id = new_matricula_id);
