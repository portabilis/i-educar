...
-- @license  @@license@@
-- @version  $Id$
-- Corrige transferência de notas quando não houver nota cadastrada na matrícula
...
$BODY$
  ...
  IF (SELECT count(1) FROM modules.nota_aluno WHERE matricula_id IN (old_matricula_id,new_matricula_id)) > 1 THEN
