...
FROM
  (SELECT ref_ref_cod_aluno AS cod_aluno,
     ...
     (SELECT CASE WHEN ((substring(trim(hd.nota),1,1) <> 0)
                        ...
                        AND (substring(trim(hd.nota),1,1) <> 9)) THEN replace(hd.nota,'.',',') WHEN ((to_number(trim(hd.nota), '999') > 10)
                                                                                                     AND (to_number(trim(hd.nota), '999') <= 20)) THEN replace(trim(hd.nota),'.',',') ELSE replace(substring(trim(hd.nota),1,4),'.',',') END
   ...
   GROUP BY ref_ref_cod_aluno,
            nm_disciplina
   ORDER BY nm_disciplina) AS historico
GROUP BY disciplina,
         cod_aluno,
         nota_1serie,
         nota_2serie,
