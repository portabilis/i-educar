...
SELECT matricula.cod_matricula,
  ...
  (SELECT CASE
              ...
              WHEN matricula.aprovado = 3
                   AND COALESCE(
                                  (SELECT mt.remanejado
                                   FROM pmieducar.matricula_turma mt
                                   WHERE mt.ref_cod_matricula = matricula_turma.ref_cod_matricula
                                     AND mt.remanejado = TRUE LIMIT 1), FALSE) THEN 'Remanejado'::character varying
              ...
              WHEN matricula.aprovado = 3
                   AND COALESCE(
                                  (SELECT mt.remanejado
                                   FROM pmieducar.matricula_turma mt
                                   WHERE mt.ref_cod_matricula = matricula_turma.ref_cod_matricula
                                     AND mt.remanejado = TRUE LIMIT 1), FALSE) THEN 'Rem'::character varying
