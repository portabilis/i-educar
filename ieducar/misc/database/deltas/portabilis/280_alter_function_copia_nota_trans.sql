...
-- @license  @@license@@
-- @version  $Id$
-- Altera função para que verifique corretamente se o aluno possui ou não notas a serem migradas
...
$BODY$
  ...
  IF (SELECT (CASE WHEN count(1) > 1 THEN FALSE ELSE TRUE END) FROM modules.nota_aluno WHERE matricula_id IN (old_matricula_id,new_matricula_id)) THEN
