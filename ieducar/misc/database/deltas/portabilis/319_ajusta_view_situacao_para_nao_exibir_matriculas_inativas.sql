-- Ajusta view de situacao para nao exibir matriculas inativas
-- autor: Paula Bonot <bonot@portabilis.com.br>
 ...
 SELECT matricula.cod_matricula, situacao_matricula.cod_situacao, matricula_turma.ref_cod_turma AS cod_turma, matricula_turma.sequencial, ( SELECT 
  ...
  WHERE 
   ...
   CASE
       ...
       ELSE matricula_turma.ativo = 1 OR matricula_turma.transferido OR matricula_turma.reclassificado OR matricula_turma.abandono OR matricula_turma.remanejado AND matricula_turma.sequencial < (( SELECT max(mt.sequencial) AS max
          FROM matricula_turma mt
         WHERE mt.ref_cod_matricula = matricula.cod_matricula))
   END AND 
   CASE
