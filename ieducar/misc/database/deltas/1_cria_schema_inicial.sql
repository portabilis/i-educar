...
BEGIN
  ...
  FOR v_reg_qtde_produto IN EXECUTE v_sql_qtde_produto LOOP
    --
    -- Verifica se o card�pio atende a unidade em quest�o
    --
...
BEGIN
  ...
  --
  -- Obt�m inscritos e matriculados para a unidade
  --
  v_num_inscritos:=0;
  ...
  END LOOP;
  --
  -- Calcula a qtde percapita necess�ria do produto para o per�odo
  --
...
CREATE FUNCTION fcn_gerar_guia_remessa(text, text, integer, integer, character varying, character varying, character varying, integer) RETURNS text
   ...
   v_qtde_disponivel numeric;         -- qtde total dispon�vel no(s) fornecedor(res)
   ...
   v_qtde_produto_periodo numeric:=0; -- qtde de produto necess�ria para o per�odo
              --(somat�ria da qtde percapita da receita)
...
BEGIN
   --
   -- Converte data num�rica invertida para o formato DD/MM/YYYY
   --
  ...
  VALUES
    ...
    ,'*** IN�CIO ***');
   --
   -- Obt�m indica��o de inscritos ou matriculados para o cliente
   --
   SELECT INTO v_inscritos inscritos
   FROM alimentos.cliente
   ...
   --
   -- Obt�m o n�mero de tipos de refei��es que ser�o atendidas
   -- (total caf� da manh�, total almo�o, etc)
   --
   ...
   --
   -- Prepara SELECT para carregar produtos existentes no card�pio
   --
   ...
   IF v_classe is not null THEN
      v_sql_produto := v_sql_produto || ' AND pro.classe = ''' || v_classe || '''';
         ...
         --
   -- Seleciona somente os produtos do card�pio que o fornecedor
         -- pode fornecedor
   --
      ...
      AND uni.iduni = ' || v_reg_unidade.iduni || '
         ...
         --
         -- Obt�m os produtos do card�pio utilizados pela unidade
         --
         ...
         FOR v_reg_produto IN EXECUTE v_sql_produto LOOP
            ...
            --
            --
            -- Calcula a qtde necess�ria para o produto e para a unidade considerando
            ...
            --
            -- Obt�m qtde do produto j� gerado em outras guias
            --
            ...
            UPDATE alimentos.log_guia_remessa SET mensagem = mensagem || ' \n      Fator corre��o: ' || TRIM(TO_CHAR(v_reg_produto.fator_correcao, '9999999999.99')) WHERE idlogguia = v_id_log;
            
            UPDATE alimentos.log_guia_remessa SET mensagem = mensagem || ' \n      Fator coc��o: ' || TRIM(TO_CHAR(v_reg_produto.fator_coccao, '9999999999.99')) WHERE idlogguia = v_id_log;
            
            UPDATE alimentos.log_guia_remessa SET mensagem = mensagem || ' \n      Peso: ' || TRIM(TO_CHAR(v_reg_produto.peso, '9999999999.99')) WHERE idlogguia = v_id_log;
            UPDATE alimentos.log_guia_remessa SET mensagem = mensagem || ' \n      Qtde necess�ria: ' || TRIM(TO_CHAR(v_qtde_necessaria, '9999999999.99')) WHERE idlogguia = v_id_log;
            
            RAISE NOTICE 'Qtde per capita(%)',v_qtde_produto_periodo;
            RAISE NOTICE 'Fator corre��o(%)',v_reg_produto.fator_correcao;
            RAISE NOTICE 'Fator coc��o(%)',v_reg_produto.fator_coccao;
            RAISE NOTICE 'Peso(%)',v_reg_produto.peso;
            RAISE NOTICE 'Qtde Necess�ria(%)',v_qtde_necessaria;
            
            --
            -- Obt�m qtde do produto em contrato, incluindo
         ...
         AND con.idfor IN (SELECT idfor
            ...
            --
            -- Testa se fornecedor tem contrato e autoriza��o para fornecer para a
            -- unidade
            --
            IF v_existe_forn = 0 THEN
               ...
               RETURN '-1 Produto: ' || v_reg_produto.nome_compra || ' n�o tem contrato ou o fornecedor contratado n�o est� autorizado a fornecer para a unidade ' || v_reg_unidade.nome || '.';
               
            END IF;
            
            --
            -- Qtde necess�ria deve ser > 0
            --
            IF v_qtde_necessaria > 0 THEN
               ...
               FOR v_reg_fornecedor IN EXECUTE v_sql_fornecedor LOOP
                  ...
                  -- Deve ser verificado por quest�es de arredondamento,
                  -- evitando enviar uma qtde maior que a necess�ria
                  
                  v_qtde_nec_saldo_ant := v_qtde_necessaria_saldo;
                  v_qtde_necessaria_saldo:=v_qtde_necessaria_saldo - v_qtde_forn;
                  
                  RAISE NOTICE 'Qtde dispon�vel(%)',v_qtde_disponivel;
                  ...
                  -- GRAVA LOG
                  --
                  UPDATE alimentos.log_guia_remessa SET mensagem = mensagem || '  \n           Qtde dispon�vel: ' || TRIM(TO_CHAR(v_qtde_disponivel, '9999999999.99')) WHERE idlogguia = v_id_log;
                  ...
                  -- Qtde dispon�vel > qtde a ser fornecida
                  ...
                  IF v_qtde_forn > 0 AND v_qtde_disponivel >= v_qtde_forn THEN
                     ...
                     IF v_id_guia IS NULL THEN
                        ...
                        -- Obt�m o pr�ximo sequencial anual para guia
         ...
         (idgui
               ...
               --
               -- Identifica que existe contrato com fornecedor, por�m
   ...
   IF v_existe_produto = 0 AND v_array_fornecedor <> '0' THEN
      ...
      -- GRAVA LOG
      --
      UPDATE alimentos.log_guia_remessa SET mensagem = mensagem || ' \n \n     *** Fornecedor n�o possui produtos para fornecer no per�odo. ***' WHERE idlogguia = v_id_log;
      
      RETURN '-1 Fornecedor n�o possui produtos para fornecer no per�odo. Verifique os contratos elaborados com o fornecedor.';
   ...
   ELSE
      ...
      -- GRAVA LOG
      --
      UPDATE alimentos.log_guia_remessa SET mensagem = mensagem || ' \n \n     *** N�o existem dados para gerar guias de acordo com os filtros informados. ***' WHERE idlogguia = v_id_log;
      
      RETURN 'N�o existem dados para gerar guias de acordo com os filtros informados.';
...
DECLARE
  ...
  /*
  consistenciacao.historico_campo.credibilidade: 1 = M�xima, 2 = Alta, 3 = M�dia, 4 = Baixa, 5 = Sem credibilidade
  cadastro.pessoa.origem_gravacao: M = Migra��o, U = Usu�rio, C = Rotina de confronta��o, O = Oscar
  */
  BEGIN
    v_idpes         := NEW.idpes;
    ...
    IF v_origem_gravacao = 'U' OR v_origem_gravacao = 'O' THEN -- os dados foram editados pelo usu�rio ou pelo usu�rio do Oscar
      v_nova_credibilidade := v_credibilidade_maxima;
    ELSIF v_origem_gravacao = 'M' THEN -- os dados foram originados por migra��o
    ...
    IF v_nova_credibilidade > 0 THEN
      ...
      -- DATA DE EMISS�O CERTID�O CIVIL
      ...
      -- DATA DE EMISS�O CARTEIRA DE TRABALHO
...
DECLARE
  ...
  */
  BEGIN
    ...
    IF v_origem_gravacao = 'U' OR v_origem_gravacao = 'O' THEN -- os dados foram editados pelo usu�rio ou usu�rio do Oscar
      v_nova_credibilidade := v_credibilidade_maxima;
    ELSIF v_origem_gravacao = 'M' THEN -- os dados foram originados por migra��o
      v_nova_credibilidade := v_credibilidade_alta;
    END IF;
...
DECLARE
  ...
  BEGIN
    -- verificar se j� existe hist�rico para o campo
...
DECLARE
  ...
  v_reg      record;
BEGIN
  --Unificando registros das tabelas do SGSP (Sistema de Gererenciamento de Servi�os P�blicos)--
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA BAIRRO
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA DOCUMENTO
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA ENDERECO_EXTERNO
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA ENDERECO_PESSOA
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA FISICA
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA FISICA_CPF
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA FUNCIONARIO
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA JURIDICA
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA LOGRADOURO
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA MUNICIPIO
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA PESSOA
...
BEGIN
  ...
  IF v_data_rev IS NULL THEN
      ...
      -- GRAVA HIST�RICO PARA TABELA SOCIO
...
DECLARE
  ...
  -- primeiro e �ltimo nome da pessoa com fon�tica
...
DECLARE
  ...
  IF v_total_caracteres > 0 THEN
    -- retirar os espa�os duplicados
    WHILE POSITION('  ' IN v_nome) > 0 LOOP
      v_nome := REPLACE(UPPER(v_nome),'  ', ' ');
    END LOOP;
    -- retirar ocorr�ncias que devem ser ignoradas no nome
    ...
    END IF;
    v_nome := TRIM(v_nome);
    -- posi��o do espa�o em branco para obter o primeiro nome
    ...
    IF v_posicao_espaco_primeiro_nome > 0 THEN
      ...
      -- obter posi��o do espa�o em branco anterior ao �ltimo nome
...
DECLARE
  ...
  IF v_total_caracteres > 0 THEN
    ...
    END IF;
    v_nome := TRIM(v_nome);
    -- posi��o do espaco em branco para obter o primeiro nome
    v_posicao_espaco_primeiro_nome := POSITION(' ' IN v_nome);
    IF v_posicao_espaco_primeiro_nome > 0 THEN
      v_primeiro_nome := SUBSTR(v_nome, 1, (v_posicao_espaco_primeiro_nome - 1));
    ELSE
...
BEGIN
  ...
  IF v_id_set_aux IS NULL AND v_id_set = -1 THEN
    -- Sql utilizado para ativar e desativar o registro na tabela funcion�rio
