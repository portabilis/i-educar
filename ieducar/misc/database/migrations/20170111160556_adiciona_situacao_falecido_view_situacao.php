...
class AdicionaSituacaoFalecidoViewSituacao extends AbstractMigration
{
    public function up()
    {
        $this->execute("DROP VIEW relatorio.view_situacao;
                        ...
                        WHERE matricula.ativo = 1
                          ...
                          AND CASE
                                  ...
                                  WHEN situacao_matricula.cod_situacao = 2 THEN (matricula.aprovado = ANY (ARRAY[2, 14]))
                                       AND (NOT matricula_turma.reclassificado
                                            OR matricula_turma.reclassificado IS NULL)
                                       AND (NOT matricula_turma.abandono
                                            OR matricula_turma.abandono IS NULL)
                                       ...
                                       AND (NOT matricula_turma.falecido
                                            OR matricula_turma.falecido IS NULL)
                                  WHEN situacao_matricula.cod_situacao = 1 THEN (matricula.aprovado = ANY (ARRAY[1, 12, 13]))
                                       AND (NOT matricula_turma.reclassificado
                                            OR matricula_turma.reclassificado IS NULL)
                                       AND (NOT matricula_turma.abandono
                                            OR matricula_turma.abandono IS NULL)
                                       ...
                                       AND (NOT matricula_turma.falecido
                                            OR matricula_turma.falecido IS NULL)
                                  WHEN situacao_matricula.cod_situacao = ANY (ARRAY[3, 12, 13]) THEN matricula.aprovado = situacao_matricula.cod_situacao
                                       AND (NOT matricula_turma.reclassificado
                                            OR matricula_turma.reclassificado IS NULL)
                                       AND (NOT matricula_turma.abandono
                                            OR matricula_turma.abandono IS NULL)
