...
class CriaViewDescricaoLogradouro extends AbstractMigration
{
    public function up()
    {
        $this->execute("DROP VIEW relatorio.view_dados_escola;

    CREATE OR REPLACE VIEW relatorio.view_dados_escola AS
        SELECT escola.cod_escola, relatorio.get_nome_escola(escola.cod_escola) AS nome,
        pessoa.email, COALESCE(endereco_pessoa.cep, endereco_externo.cep) AS cep,
        ...
        educacenso_cod_escola.cod_escola_inep AS inep, relatorio.get_ddd_escola(escola.cod_escola) AS telefone_ddd,
